...
import { FIELD_AREA, SLUG_RE, getField, readText, scanTasks, setField, writeText } from './tasks.js';
import { run as newTask } from './new.js';
import { CliError, info, ok, parseCommandArgs, toPosix, warn, worktrees } from './util.js';
...
function walk(root, rel, exts, lang) {
  ...
  const isLinkedWorktree = (abs) => {
    if (!isFile(path.join(abs, '.git'))) return false;
    linked ??= new Set(worktrees(root, lang).map((w) => realOrNull(w.path)));
    return linked.has(realOrNull(abs));
  };
  const out = [];
