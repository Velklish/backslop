...
import { EXTERNAL, normalizeHrefTarget, splitHref } from './links.js';
...
async function runQueueReference(project) {
  ...
  for (const line of readText(index).split('\n')) {
    ...
    const rel = normalizeHrefTarget(toPosix(path.relative(project.root, dirs.reference)), target);
    if (rel !== null && existsSync(path.join(project.root, rel))) continue;
    const slug = `describe-${path.basename(target, '.md').toLowerCase()}`;
