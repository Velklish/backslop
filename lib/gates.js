...
// снимка нет — это не отказ, а отсутствие улики, и оно называется вслух.
function treeSnapshot(root) {
  const head = git(root, ['rev-parse', 'HEAD']);
  if (head.status !== 0) return null;
  const status = git(root, ['status', '--porcelain']);
  if (status.status !== 0) return null;
  return { head: head.stdout.trim(), clean: status.stdout.trim() === '', dirty: status.stdout.trim() };
}
...
export async function run(argv, { cwd }) {
  ...
  if (asJson) {
    ...
    else info(tr(cfg.lang, `дерево: ${tree.head}, ${tree.clean ? 'чисто' : 'нечисто'}`, `tree: ${tree.head}, ${tree.clean ? 'clean' : 'dirty'}`));
  }
