...
function changedPaths(root, base, prefix, lang) {
  // Both names of a rename count: the scope a file left is touched too (ADR-023).
...
export async function run(argv, { cwd }) {
  ...
  // git paths start at the repository root, `when` globs at `backslop.json` (ADR-023). Resolved
  ...
  for (const gate of gates) {
    ...
    // A tool failure is not called a red gate (ADR-009); the report keeps the fields it always had.
