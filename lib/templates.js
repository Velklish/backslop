...
// Ключи, которые код кладёт в шаблон, — по группам вызывающих: `init` и `adapters` идут
// одним набором на весь слой docs/skills, остальные команды — своим на файл. Пара
// «плейсхолдер ↔ ключ» иначе расходится молча, и `{{…}}` уезжает читателю буквально.
export const TEMPLATE_KEYS = [
  [/^(?:docs\/|skills\/|agents-section\.md$)/, ['adrNumber', 'cli', 'date', 'docs', 'prefix', 'project']],
  [/^adr\.md$/, ['date', 'number', 'title']],
  [/^result\.md$/, ['date', 'id', 'prefix']],
  [/^task\.md$/, ['area', 'context', 'date', 'id', 'title']],
  [/^brief\.md$/, ['autonomy', 'cli', 'entry', 'gates', 'handover', 'measurements', 'neighbours', 'prefix', 'tasks', 'track']],
];
...
// Пара «плейсхолдер ↔ ключ». Имя в шаблоне без ключа в `TEMPLATE_KEYS` — дырка, которую
// видит только читатель брифа; объявленный ключ, которого нет ни в одном шаблоне своей
// группы, — вторая половина той же дырки: слот завели в коде и забыли в тексте. Сверка
// механическая и идёт по обоим языковым слоям: en-шаблон, разошедшийся с ru, иначе
// проскочил бы мимо этого гейта в дыру гейта парности.
