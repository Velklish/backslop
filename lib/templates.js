...
export function templateParity(root = TEMPLATES_DIR, lang = 'ru') {
  ...
  for (const rel of ru.keys()) if (!en.has(rel)) errors.push(tr(lang, `templates/en/${rel} нет`, `templates/en/${rel} is missing`));
  for (const rel of en.keys()) if (!ru.has(rel)) errors.push(tr(lang, `templates/en/${rel} — нет исходника в templates/`, `templates/en/${rel} has no source counterpart`));
  for (const [rel, ruFile] of ru) {
    ...
    if (ruVars.join('\0') !== enVars.join('\0')) {
      ...
      errors.push(tr(lang, `templates/en/${rel}: подстановки расходятся: ${enVars.join(', ') || none} != ${ruVars.join(', ') || none}`, `templates/en/${rel} placeholders differ: ${enVars.join(', ') || none} != ${ruVars.join(', ') || none}`));
    }
    ...
    if (ruHeads !== enHeads) {
      const none = tr(lang, '(нет)', '(none)');
      errors.push(tr(lang, `templates/en/${rel}: заголовки расходятся: ${enHeads || none} != ${ruHeads || none}`, `templates/en/${rel} headings differ: ${enHeads || none} != ${ruHeads || none}`));
    }
  }
