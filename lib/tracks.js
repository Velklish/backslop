...
import { CliError, git, info, ok, parseCommandArgs } from './util.js';
import { tr } from './i18n.js';

// Блоки `git worktree list --porcelain` разделены пустой строкой; первая строка блока — путь,
// строки `branch` у detached HEAD нет, а строка `HEAD <sha>` есть всегда — по ней и меряется
// detached: «влитость не проверена» там, где git готов ответить, было бы отговоркой.
// Тот же разбор вшит внутрь цикла `foreignTaskIds` (lib/tasks.js) и оттуда не отдаётся —
// см. находку BS-32.1.
...
export async function run(argv, { cwd }) {
  ...
  for (const wt of worktrees(root)) {
    ...
    // Detached worktree меряется по своему sha: ветки у него нет, а работа в нём — есть.
