...
// цитирует версию как улику того, что было, и переписывать её нельзя. Для живых пинов расширенное
// множество ниже объединяет этот обход с `package.json` и файлами CI.
...
// Живые файлы с исполняемым пином: markdown из liveMarkdown, package.json и файлы известных
// CI-каталогов. История остаётся только в markdown-исключениях выше; конфигурация сборки и
// манифесты всегда описывают текущий запуск и должны подниматься одной командой.
...
export function livePinFiles(root, docs, prefix) {
  ...
  for (const file of srcFiles(root, '', ['.json'], [], null, linkedHarnessRoots(root))) {
    if (path.posix.basename(file[0]) === 'package.json') add(file);
  }
  for (const name of CI_ROOT_FILES) {
    const abs = path.join(root, name);
    if (statOrNull(abs)?.isFile()) add([name, abs]);
  }
  for (const [dir, exts] of CI_DIRS) {
    for (const file of srcFiles(path.join(root, dir), dir, exts)) add(file);
  }
  return files;
}
