...
export function rootMarkdown(root, walked = []) {
  ...
  if (entries.some((e) => e.isSymbolicLink())) {
    for (const [, abs] of walked) seen.add(realOrNull(abs));
    for (const e of entries) if (e.isFile()) seen.add(realOrNull(path.join(root, e.name)));
  }
  const out = [];
  for (const e of entries) {
    const abs = path.join(root, e.name);
    if (e.isSymbolicLink()) {
      const real = insideProject(root, abs) && statOrNull(abs)?.isFile() ? realOrNull(abs) : null;
...
export function liveMarkdown(root, docs, prefix) {
  ...
  const card = new RegExp(`^${esc(prefix)}-\\d+(?:\\.\\d+)?-.+\\.[Mm][Dd]$`);
