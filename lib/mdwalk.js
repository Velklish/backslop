...
// A directory the walk cannot list. The message is the path and the code; a command that knows
// the project language words it with `tr`.
export class UnreadableDir extends CliError {
  constructor(dir, code) {
    super(`${dir}: ${code}`);
    this.dir = dir;
    this.code = code;
  }
...
// Every stale pin of the live files, read-only: a `pinRe` match off `form.pin` outside a journal
// entry. The lint gate reports these; `upgrade` rewrites them with a raw read of its own.
