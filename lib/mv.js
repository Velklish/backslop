...
import {
  FIELD_ORDER, FIELD_TAKEN, SECTION_DEFERRED, fieldName, findFlatTask, findTask, formatId, moveFile, parseId, placeInQueue,
...
export async function run(argv, { cwd }) {
  ...
  const outgoing = reorder ? text : rewriteMovedLinks(text, path.posix.dirname(task.rel), path.posix.dirname(newRel));
  const outgoingChanged = outgoing !== text;
  text = outgoing;
