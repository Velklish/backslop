...
import { isTodoPlaceholder } from './lint.js';
import {
  ...
  eolOf, fieldName, findFlatTask, findTask, formatId, parseId, placeInQueue, getField, orderOf, queueOrder, readText, relocateTask, removeField, sameId, scanTasks,
  sectionBody, sectionName, appendSection, setField, splitLines, writeText,
} from './tasks.js';
...
// остаётся, «Контекст» без «Улики» становится ею, `--evidence` пишет строку улики, как `new --minor`.
function minorForm(text, evidence, lang) {
  let lines = splitLines(text);
  const cut = (i) => lines.splice(i, !lines[i - 1]?.trim() && !lines[i + 1]?.trim() ? 2 : 1);
  const sections = () => {
    const clean = splitLines(blankFences(lines.join('\n')));
    const out = [];
    ...
    out.forEach((sec, k) => {
      sec.end = k + 1 < out.length ? out[k + 1].start : lines.length;
      sec.stubs = [];
      sec.filled = false;
  ...
  let section = sections().find((sec) => evidenceNames.has(sec.name));
  const context = sections().find((sec) => names(SECTION_CONTEXT).has(sec.name));
  ...
  section = sections().find((sec) => evidenceNames.has(sec.name));
  if (evidence) {
...
const EVIDENCE_REQUIRED_EN = (ids) => [
  ...
  'Give it with the flag: mv N minor --evidence "…" — a path with a line, a command with its output and exit code, or a measurement with a number.',
...
export async function run(argv, { cwd }) {
  ...
  if (!rawIds.length || !target) throw new CliError(tr(cfg.lang, `нужны номер и статус: backslop mv <N…> <${STATUSES.join('|')}> [--top | --after M | --restore] [--evidence "…"]`, `task numbers and a status are required: backslop mv <N…> <${STATUSES.join('|')}> [--top | --after M | --restore] [--evidence "…"]`));
  ...
  if (bare.length) throw new CliError(tr(cfg.lang, EVIDENCE_REQUIRED_RU(bare), EVIDENCE_REQUIRED_EN(bare)));
