...
import {
  FIELD_AREA, FIELD_COST, FIELD_ORDER, FIELD_TAKEN, SECTION_DEFERRED, fieldName, findFlatTask, findTask, formatId, parseId, placeInQueue,
  getField, queueOrder, readText, relocateTask, removeField, sameId, scanTasks, sectionBody, appendSection, setField, writeText,
} from './tasks.js';
...
export async function run(argv, { cwd }) {
  ...
  if (!rawIds.length || !target) throw new CliError(tr(cfg.lang, `нужны номер и статус: backslop mv <N…> <${STATUSES.join('|')}> [--top | --after M]`, `task numbers and a status are required: backslop mv <N…> <${STATUSES.join('|')}> [--top | --after M]`));
  ...
  for (const task of planned) {
    const moved = moveOne(project, task, target, values);
    if (moved.costAdded) costAdded += 1;
    renumbered += moved.renumbered.length;
