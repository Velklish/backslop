...
function moveOne(project, task, target, values, saved, before, evidence) {
  ...
  if (reorder) {
    ok(tr(cfg.lang, `${task.id}: queue/ «${fieldName(FIELD_ORDER, cfg.lang)}» ${placed.rank}`, `${task.id}: queue/ “${fieldName(FIELD_ORDER, cfg.lang)}” ${placed.rank}`));
