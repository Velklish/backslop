...
function moveOne(project, task, target, values, saved) {
  ...
  if (target === 'queue') {
    ...
    placed = placeInQueue(rows, { top: Boolean(values.top), after, rank: saved }, cfg.lang);
  }
...
export async function run(argv, { cwd }) {
  ...
  // Порядок обработки пакета — по убыванию сохранённого числа, а не по порядку аргументов:
  // `mv 9 3` иначе дал бы не то, что `mv 3 9`. Убывание, а не возрастание: место каждой задачи
  // считается заново, и задача с занятым числом встаёт ПЕРЕД занявшим — расталкивая тесную
  // очередь, она обгоняет соседа по пакету, которого при возрастании уже поставили. Полной
  // гарантии порядка убывание не даёт — остаток в BS-30.1; оно снимает класс инверсий, а не все.
  // При равных числах решает номер задачи, в ту же сторону: в очереди они встают по возрастанию.
  ...
  let costAdded = 0;
  for (const task of planned) {
    const moved = moveOne(project, task, target, values, restoring?.get(task.file) ?? null);
