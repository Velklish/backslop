...
import { blankCode, blankFences, rewriteIncomingLinks, rewriteMovedLinks } from './links.js';
import { LOG_FILE, batchOf, logFile, readLogText } from './log.js';
import { repoMarkdown } from './mdwalk.js';
import { CliError, git, gitOrFail, toPosix, warn, worktrees } from './util.js';
...
// Все задачи проекта по номеру: каталог на статус, несвёрнутые каталоги архива и строки журнала
// закрытых. Два источника архива читаются вместе и живут рядом сколько угодно долго: свёртка
// накопленного — отдельный ход, а не условие работоспособности ([ADR-026](../docs/adr/adr-026-archive-folds-to-log.md)).
// Файл, не подходящий под шаблон имени, здесь не задача — его ловит гейт `lint`, а не команды.
...
// Свёрнутые задачи: запись журнала — та же задача в статусе archive, только тела в дереве у неё
// нет. `file` и `rel` указывают на сам журнал: диагностика гейтов адресует место, где запись
// лежит, а `hasTask: false` уже понимают потребители, которым нужна постановка (`brief`).
...
// Каталог, названный как файл задачи, — не задача: его называет гейт 2 `lint` («каталог внутри
// каталога статуса»), а чтение каталога как файла роняло команду стеком EISDIR раньше, чем гейт
// доходил до своей ветви. Symlink на файл задачей остаётся — читается сквозь ссылку, как и
// раньше; symlink на каталог получает у гейта 2 ту же диагностику, что каталог. Всё, что не
// каталог и не файл по Dirent, доразрешается stat: битая ссылка задачей не считается, её
// называет гейт 2.
...
export function foreignTaskIds({ root, cfg }, lang = 'ru') {
  ...
  if (branches.status === 0) {
    for (const branch of branches.stdout.split('\n').map((b) => b.trim()).filter(Boolean)) {
      ...
      // Имён файлов у свёрнутой задачи нет — номер занят строкой журнала, и его видно только в
      // содержимом. Одно чтение на ветку: без него worker в worktree выдал бы номер, уже занятый
      // закрытой задачей соседней ветки, и столкновение нашлось бы только при слиянии.
      const log = git(root, ['show', `${branch}:${docsRel}/archive/${LOG_FILE}`]);
      if (log.status === 0) fromLog(log.stdout, source);
    }
  }
  return out;
}
...
// Место в очереди: сверху, после задачи M, на сохранённый ранг или в конец. Между соседями
// берётся середина; когда целого места нет — очередь перенумеровывается шагом RANK_STEP,
// порядок сохраняется. Возвращает { rank, renumbered: [[file, rank], …] }.
export function placeInQueue(rows, { top = false, after = null, rank = null } = {}, lang = 'ru') {
  ...
  if (rank !== null) {
    ...
    const idx = ordered.findIndex((r) => r.rank === rank);
    if (idx === -1) return { rank, renumbered: [] };
    insertBefore = idx;
    lower = idx === 0 ? 0 : ordered[idx - 1].rank;
    upper = rank;
  } else if (top) {
    insertBefore = 0;
    lower = 0;
