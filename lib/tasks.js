...
import { STATUSES, expectNameFits } from './config.js';
...
import {
  CliError, git, gitCause, gitOrFail, insideRepo, isSameTree, isTracked, localBranches, readText, realpathOrNull, toPosix, today, warn, worktrees, writeText,
} from './util.js';
...
export function formatCost(level, hypothesis, lang = 'ru') {
  return hypothesis ? `${level} (${tr(lang, 'гипотеза', 'hypothesis')})` : level;
}
...
export function parseId(raw, prefix, lang = 'ru') {
  ...
  if (!id) throw new CliError(tr(lang,
...
function areaPlaceholder(lang, reference) {
  if (reference === null) return tr(lang, '[TODO: раздел reference/]', '[TODO: reference/ section]');
  return tr(lang, `[TODO: раздел](${reference})`, `[TODO: section](${reference})`);
}
...
export function createTask(project, { slug, title, queue, top, parent: parentRef, minor, cost = 'minor', hypothesis, evidence = '' }) {
  ...
  let context = tr(cfg.lang, '[TODO: откуда задача и что за ней стоит.]', '[TODO: where the task came from and what motivates it.]');
  ...
  // Номер находки наследует форму родителя (`BS-007` → `BS-007.1`). У родителя-находки ребёнок —
  // следующий `N.k` того же корня (`BS-007.1` → `BS-007.2`), точное ребро — в поле «Родитель».
  ...
  // Глубину пути до справочника считает команда, раздел выбирает тот, кто заводит задачу. Без
  // справочника заглушка остаётся текстом: битая ссылка красила бы гейт 1.
  ...
  writeText(file, text);
  const notes = [];
  if (blocker) notes.push(tr(cfg.lang, `${formatId(cfg.prefix, blocker.num, blocker.sub)} занят: ${blocker.source}`, `${formatId(cfg.prefix, blocker.num, blocker.sub)} is taken: ${blocker.source}`));
  if (renumbered.length) notes.push(tr(cfg.lang, `очередь перенумерована шагом 10: ${renumbered.length} файлов`, `queue renumbered in steps of 10: ${renumbered.length} files`));
