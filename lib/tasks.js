...
export function getField(text, name) {
  ...
  for (const alias of fieldAliases(name)) {
    if (fields.has(alias)) return fields.get(alias);
  }
  return null;
}
...
export function setField(text, name, value, lang = 'ru') {
  ...
  const own = fields.find(([, n]) => aliases.has(n));
  if (own) {
    lines[own[0]] = line;
...
export function removeField(text, name) {
  ...
  const lines = splitLines(text);
  const aliases = fieldAliases(name);
  const own = fieldLines(lines).find(([, n]) => aliases.has(n));
  if (!own) return text;
