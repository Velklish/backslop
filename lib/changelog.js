...
import { projectLangOrNull } from './config.js';
...
export async function run(argv, { cwd }) {
  ...
  if (values.since !== undefined && since === null) throw new CliError(pick(lang, `--since «${values.since}»: нужна форма X.Y.Z`, `--since “${values.since}”: expected X.Y.Z`, `--since “${values.since}”: expected X.Y.Z / нужна форма X.Y.Z`));
  if (values.to !== undefined && to === null) throw new CliError(pick(lang, `--to «${values.to}»: нужна форма X.Y.Z`, `--to “${values.to}”: expected X.Y.Z`, `--to “${values.to}”: expected X.Y.Z / нужна форма X.Y.Z`));
  const text = changelogSince(readFileSync(CHANGELOG_FILE, 'utf8'), since, to);
  const empty = pick(lang,
    `записей ${since ? `после v${since} и ` : ''}до v${to} нет\n`,
