// Бриф worker'у: из обязательных пунктов большинство — данные с диска и неизменный текст,
// решения оркестратора остаются текстовыми аргументами — границы правки, точка входа,
// граница самостоятельности и форма сдачи. Команда печатает текст в stdout; куда его
// отправить — промпт субагента, первое сообщение сессии, шина — решает оркестратор.
import { loadProject, parseCli } from './config.js';
...
// Раннер гейтов появился в v0.5.0, а бриф рендерится версией, которую пинует проект: на
// пине старше раннера шаг «позови gates» неисполним, и worker платит ходом за разбор.
// Остальные команды блока (`new --parent`) есть с v0.1.0 — проверять там нечего.
const GATES_SINCE = '0.5.0';
...
export async function run(argv, { cwd }) {
  ...
  if (!positionals.length) {
    throw new CliError(tr(cfg.lang,
      'нужны номера задач: backslop brief <N…> [--track "…"] [--neighbour "путь=track"] [--entry "…"] [--autonomy "…"] [--handover "…"] [--measurements]',
      'task numbers are required: backslop brief <N…> [--track "…"] [--neighbour "path=track"] [--entry "…"] [--autonomy "…"] [--handover "…"] [--measurements]'));
  }
...
// Шаг гейтов — по тому, что умеет пинованная версия, а не запущенная. Пина нет (self-host,
// глобальная установка) — раннер есть: проект зовёт тот же CLI, который сейчас рендерит.
function pinHasGates(cfg) {
  const form = parseCli(cfg.cli);
  return !form || form.pin === null || compareVersions(form.pin, GATES_SINCE) >= 0;
}
...
function gatesStep(cfg) {
  ...
  if (!cfg.gates.length) {
    return tr(lang,
      'Гейты проекта зелёные числом, но состав `gates` в `backslop.json` пуст — спроси оркестратора, чем проверяется задача.',
      'Project gates are green by count, but the `gates` list in `backslop.json` is empty — ask the orchestrator what checks this task.');
  }
  const list = cfg.gates.map((g) => `\`${g}\``).join(', ');
  if (pinHasGates(cfg)) {
    return tr(lang,
      `Гейты проекта зелёные числом: \`${cfg.cli} gates\` печатает итог «гейтов N, зелёных N». Их состав — ${list}. Код возврата смотри у команды, а не у пайпа: \`cmd > out; echo $?\`.`,
      `Project gates are green by count: \`${cfg.cli} gates\` prints the summary “gates N, green N”. Their contents: ${list}. Read the exit code of the command, not of a pipe: \`cmd > out; echo $?\`.`);
  }
  return tr(lang,
    `Гейты проекта зелёные — все до одного: ${list}. Раннера \`gates\` в пинованной версии нет (команда появилась в v${GATES_SINCE}), поэтому гони их по одной и приложи код возврата каждой: \`cmd > out; echo $?\` — у команды, а не у пайпа. Сводку «гейтов N, зелёных N» не выдумывай: её печатает раннер, которого на этом пине нет.`,
    `Project gates are green — every one of them: ${list}. The pinned version has no \`gates\` runner (the command arrived in v${GATES_SINCE}), so run them one by one and attach the exit code of each: \`cmd > out; echo $?\` — of the command, not of the pipe. Do not invent the “gates N, green N” summary: it is printed by a runner this pin does not carry.`);
}
