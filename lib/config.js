...
export function pinRe(form) {
  ...
  return new RegExp(`${spec}${sep}(\\d+\\.\\d+\\.\\d+)`, 'g');
}
...
// The message language without validating the config: a broken one gives `ru`, no project `null`.
export function projectLangOrNull(cwd) {
  const root = findRoot(cwd);
  if (!root) return null;
  return readJsonOrNull(path.join(root, CONFIG_FILE))?.lang === 'en' ? 'en' : 'ru';
}
...
export function validateConfig(cfg, lang) {
  if (typeof cfg.prefix !== 'string' || !PREFIX_RE.test(cfg.prefix)) {
    ...
    throw new CliError(tr(lang, `${CONFIG_FILE}: prefix «${shown}» — нужны 2–6 заглавных латинских букв или цифр, первая буква`, `${CONFIG_FILE}: prefix “${shown}” — expected 2–6 uppercase Latin letters or digits, starting with a letter`));
  }
  ...
  if (cfg.lang === undefined) throw missingField('lang', 'нужен ru или en', 'must be ru or en', null);
  ...
  if (cfg.tools === undefined) {
    throw missingField('tools', 'нужен список без повторов из claude, cursor, codex, [] — без adapters', 'expected a unique array of claude, cursor, codex, [] for no adapters', lang);
  }
  if (!Array.isArray(cfg.tools) || !validTools(cfg.tools)) {
    throw new CliError(tr(lang, `${CONFIG_FILE}: tools — список без повторов из claude, cursor, codex`, `${CONFIG_FILE}: tools must be a unique array of claude, cursor, codex`));
  }
...
function missingField(field, ru, en, lang) {
  const texts = [
    `поля ${field} нет — ${ru}; допиши его в ${CONFIG_FILE} руками: init сначала читает конфиг и сам поле не добавит`,
    `${field} is missing — ${en}; add it to ${CONFIG_FILE} by hand: init reads the config first and cannot add the field`,
  ];
  return new CliError(`${CONFIG_FILE}: ${lang ? tr(lang, ...texts) : texts.join(' / ')}`);
}
...
export function validateBlockValue(value, label, lang) {
  ...
  if (value.includes('`')) {
    throw new CliError(pick(lang, `${label} — значение без обратной кавычки: шаблон ставит его в код-спан, и кавычка внутри его закрывает`, `${label} must not contain a backtick: the template puts it in a code span, and a backtick inside closes it`));
  }
  if (BLOCK_MARKER_RE.test(value)) {
    throw new CliError(pick(lang, `${label} — значение без меток backslop:start и backslop:end: оно стоит внутри блока, а границы блока ищутся по сырому тексту`, `${label} must not contain the backslop:start or backslop:end markers: it sits inside the block, and the block bounds are found in raw text`));
  }
}
...
function assertSingleLine(value, label, lang) {
  ...
  throw new CliError(pick(lang, `${label} — однострочное значение без переводов строк: вторая строка встаёт в блоке отдельным абзацем`, `${label} must be a single-line value without line breaks: a second line becomes a separate paragraph inside the block`));
}
...
function validateGates(cfg, lang) {
  ...
  cfg.gates.forEach((gate, i) => {
    ...
    if (typeof gate === 'string') {
      if (!gate.trim()) throw new CliError(tr(lang, `${at} — непустая строка команды`, `${at} must be a non-empty command string`));
      return;
    }
...
export function expectDirectory(root, dir, lang) {
  ...
  for (const part of path.relative(root, dir).split(path.sep).filter(Boolean)) {
    ...
    throw new CliError(tr(lang, `${rel} — файл, а нужен каталог`, `${rel} is a file, expected a directory`));
  }
}
...
export function expectNameFits(name, lang) {
  ...
  if (bytes <= 255) return;
  throw new CliError(tr(lang, `slug слишком длинный: имя файла ${bytes} байт, предел файловой системы — 255`, `slug is too long: the file name is ${bytes} bytes, the file system limit is 255`));
}
