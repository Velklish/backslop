...
// Форма значения, а не только тип: `docs`, `cli` и `probe` уезжают в managed-блок, и форма у них
// там одна на всех. Проверка тоже одна: копия запретов у каждого поля переживает появление
// следующего и оставляет его открытым — так `cli` и остался без запрета меток, когда его получил
// `probe` (BS-57.1). Что закрывает каждый запрет и почему `<` среди них нет — ADR-021.
// `prefix` в блок тоже уезжает, но сюда не идёт: PREFIX_RE не пропускает ни одного из этих
// символов, и вызов был бы недостижимым кодом.
...
// Запись `gates` — либо строка-команда, которая гоняется всегда, либо та же команда с областью:
// `{ command, when: [<glob>…] }`. Один вид у обеих форм, чтобы раннер и бриф не разбирали тип
// у себя; строка остаётся законной и ведёт себя как прежде (ADR-023).
