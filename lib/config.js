...
// Пин в прозе: та же спека, что в `cli`, с любым номером версии. Одним шаблоном ходят перепись
// в `upgrade` и ошибка в `lint`: разойдясь, один назвал бы то, чего другой не чинит.
// Номер захватывается целиком (`#v0.1.09` — один номер, а не `0.1.0` и хвост), поэтому замена
// не собирает из старого и нового мусорную версию.
...
// Форма значения, а не только тип: оно уезжает в managed-блок. Что закрывает каждый запрет и
// почему `<` среди них нет — ADR-021.
function validateProbe(cfg, lang) {
  ...
  if (LINE_BREAK_RE.test(cfg.probe)) {
    throw new CliError(tr(lang, `${CONFIG_FILE}: probe — однострочная команда без переводов строк`, `${CONFIG_FILE}: probe must be a single-line command without line breaks`));
  }
  if (cfg.probe.includes('`')) {
    throw new CliError(tr(lang, `${CONFIG_FILE}: probe — команда без обратной кавычки: шаблон ставит значение в код-спан, и кавычка внутри его закрывает`, `${CONFIG_FILE}: probe must not contain a backtick: the template puts the value in a code span, and a backtick inside closes it`));
  }
  if (BLOCK_MARKER_RE.test(cfg.probe)) {
    throw new CliError(tr(lang, `${CONFIG_FILE}: probe — команда без меток backslop:start и backslop:end: значение стоит внутри блока, а границы блока ищутся по сырому тексту`, `${CONFIG_FILE}: probe must not contain the backslop:start or backslop:end markers: the value sits inside the block, and the block bounds are found in raw text`));
  }
}
