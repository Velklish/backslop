...
export function validateBlockValue(value, field, lang) {
  assertSingleLine(value, `${CONFIG_FILE}: ${field}`, lang);
  if (value.includes('`')) {
...
// Every value that lands in the managed block: `label` names the field in the refusal.
function assertSingleLine(value, label, lang) {
  if (!LINE_BREAK_RE.test(value)) return;
  throw new CliError(tr(lang, `${label} — однострочное значение без переводов строк: вторая строка встаёт в блоке отдельным абзацем`, `${label} must be a single-line value without line breaks: a second line becomes a separate paragraph inside the block`));
}
