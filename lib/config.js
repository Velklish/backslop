...
export const TOOLS = ['claude', 'cursor', 'codex'];
...
// из npm-формы не выводится: его явно задаёт поле source. Глобальная команда
...
export function loadConfig(root) {
  ...
  const legacyTools = raw.tools === undefined
    && existsSync(path.join(root, '.claude', 'skills', 'backslop-task', 'SKILL.md'))
    ? ['claude']
    : base.tools;
