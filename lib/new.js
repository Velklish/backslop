...
import {
  FIELD_ORDER, FIELD_PARENT, SLUG_RE, findTask, foreignTaskIds, formatId, nextNumber, nextSub, parseId, placeInQueue,
...
export async function run(argv, { cwd }) {
  ...
  if (!slug) throw new CliError(tr(cfg.lang, 'нужен slug: backslop new <slug> [--title "…"] [--queue [--top]] [--parent N[.M]]', 'slug is required: backslop new <slug> [--title "…"] [--queue [--top]] [--parent N[.M]]'));
  ...
  if (values.parent !== undefined) {
    ...
    context = cfg.lang === 'en'
      ? `Finding discovered while working on ${parentTask.id}. Evidence: [TODO: file path or command output]. Quote a file inside a “<!-- quote:path --> … <!-- /quote -->” block rather than by line number: numbers drift silently, the block is guarded by lint. If unverified, state it as an assumption.`
      : `Находка при работе над ${parentTask.id}. Улика: [TODO: путь к файлу или команда с выводом]. Цитату файла оборачивай в блок «<!-- quote:путь --> … <!-- /quote -->», а не давай номером строки: номера уезжают молча, за блоком следит lint. Не проверено — написать как предположение.`;
  } else {
    num = nextNumber(all);
  ...
  let text = renderProjectTemplate(cfg, 'task.md', {
    ...
    date: today(),
    area: areaPlaceholder(cfg.lang, reference),
    context,
  });
  if (parentIsFinding) text = setField(text, FIELD_PARENT, parentTask.id, cfg.lang);
  const renumbered = [];
  if (status === 'queue') {
