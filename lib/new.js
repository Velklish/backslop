...
import { expectNameFits, loadProject } from './config.js';
import { formatId } from './ids.js';
import { renderProjectTemplate } from './templates.js';
import {
  COST_LEVELS, FIELD_ORDER, FIELD_PARENT, SLUG_RE, evidenceAssumption, findTask, foreignTaskIds, formatCost, nextNumber, nextSub, parseId, placeInQueue,
  queueOrder, scanTasks, setField,
} from './tasks.js';
...
export async function run(argv, { cwd }) {
  ...
  const vars = { id, title: (values.title ?? '').trim() || slug, date: today(), area: values.minor ? '' : areaPlaceholder(cfg.lang, reference), context };
  ...
  const renumbered = [];
  if (status === 'queue') {
    const placed = placeInQueue(queueOrder(tasks), { top: Boolean(values.top) }, cfg.prefix, cfg.lang);
