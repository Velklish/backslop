...
export async function run(argv, { cwd }) {
  ...
  if (!slug) throw new CliError(tr(cfg.lang, 'нужен slug: backslop new <slug> [--title "…"] [--queue [--top]] [--parent N[.M] [--minor [--cost <уровень>] [--hypothesis]]]', 'slug is required: backslop new <slug> [--title "…"] [--queue [--top]] [--parent N[.M] [--minor [--cost <level>] [--hypothesis]]]'));
  ...
  if ((values.cost !== undefined || values.hypothesis) && !values.minor) throw new CliError(tr(cfg.lang, '--cost и --hypothesis имеют смысл только вместе с --minor', '--cost and --hypothesis are only valid together with --minor'));
