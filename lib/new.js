...
export async function run(argv, { cwd }) {
  ...
  if (values.parent !== undefined) {
    ...
    if (!findTask(tasks, parent, cfg.lang)) throw new CliError(cfg.lang === 'en'
    ...
    context = cfg.lang === 'en'
      ? `Finding discovered while working on ${formatId(cfg.prefix, num)}. Evidence: [TODO: file path or command output]. If unverified, state it as an assumption.`
      : `Находка при работе над ${formatId(cfg.prefix, num)}. Улика: [TODO: путь к файлу или команда с выводом]. Не проверено — написать как предположение.`;
  } else {
