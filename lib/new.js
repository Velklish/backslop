...
export async function run(argv, { cwd, lang }) {
  ...
  if (!slug) throw new CliError(tr(cfg.lang, `нужен slug: ${cfg.cli} new <slug> [--title "…"] [--queue [--top]] [--parent N[.M] [--minor --evidence "…" [--cost <уровень>] [--hypothesis]]]`, `slug is required: ${cfg.cli} new <slug> [--title "…"] [--queue [--top]] [--parent N[.M] [--minor --evidence "…" [--cost <level>] [--hypothesis]]]`));
  ...
  if (!values.minor && !values.queue) info(tr(cfg.lang, `запись лежит в triage/ до разбора; в очередь — ${cfg.cli} mv <N> queue`, `entry remains in triage/ until review; move it to the queue with ${cfg.cli} mv <N> queue`));
  if (values.minor) info(tr(cfg.lang, `запись лежит в minor/ до пачки: пачка — карточка со списком номеров, закрытие — ${cfg.cli} archive N.k --into M`, `entry remains in minor/ until a batch: a batch is a card listing the numbers, closed with ${cfg.cli} archive N.k --into M`));
  return 0;
}
