...
import { CliError, bad, parseCommandArgs, toPosix, warn } from './util.js';
...
function unreleasedIds(text) {
  const { sections } = splitSections(text);
  const at = unreleasedIndex(sections);
  if (at === -1) return new Set();
  return new Set(indexBlocks(parseSection(sections[at].lines)).keys());
}
...
export function mergeChangelog(oursText, theirsText, baseText = null, lang = 'ru') {
  ...
  const baseIds = baseText === null ? null : unreleasedIds(baseText);

  const report = { ours: countEntries(oursSection), theirs: countEntries(theirsSection), conflicts: [], onlyOurs: [], onlyTheirs: [], onlyOursPlain: [], onlyTheirsPlain: [], dropped: [], duplicatesOurs: repeatedBlocks(oursSection), duplicatesTheirs: repeatedBlocks(theirsSection) };
  const merged = [];
  const byKey = new Map();
  const seen = new Set();
