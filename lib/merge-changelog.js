...
import { findRoot, projectLangOrNull } from './config.js';
...
// has no tag, as `release --bump` leaves it before the release (ADR-030).
...
export function mergeChangelog(oursText, theirsText, baseText = null, lang = 'ru', released = () => true, eol = eolOf(oursText)) {
  ...
  if (oursAt === -1) {
    throw new CliError(pick(lang,
      `${CHANGELOG_FILE} стороны --ours: нет секции невыпущенного (заголовок «## …», который не начинается с номера версии, или верхняя секция с версией без тега)`,
      `${CHANGELOG_FILE} on the --ours side has no unreleased section (a “## …” heading that does not start with a version, or a top section whose version has no tag)`));
  }
...
function readRevision(root, ref, lang) {
  ...
  if (r.status !== 0) {
    throw new CliError(pick(lang, `не читается ${ref}:${CHANGELOG_FILE} — ${gitCause(r, 'ru')}`, `cannot read ${ref}:${CHANGELOG_FILE} — ${gitCause(r, 'en')}`));
  }
  return r.stdout;
}
...
function taggedVersions(root, lang) {
  ...
  if (r.status !== 0) {
    throw new CliError(pick(lang, `не читается список тегов — ${gitCause(r, 'ru')}`, `cannot read the tag list — ${gitCause(r, 'en')}`));
  }
...
function onOut(file, lang, step) {
  try {
    ...
    throw new CliError(pick(lang, `не записывается --out ${file}: ${e.code ?? e.message}`, `cannot write --out ${file}: ${e.code ?? e.message}`));
  }
}
...
export async function run(argv, { cwd }) {
  ...
  const lang = projectLangOrNull(cwd);
  ...
  if (report.section.theirs === null) {
    note(pick(lang, 'у theirs нет секции невыпущенного — записи theirs не читались', 'theirs has no unreleased section — its entries were not read'));
  }
  for (const [side, title] of Object.entries(report.section)) {
    if (title !== null && sectionVersion(title) !== null) {
      note(pick(lang, `секция невыпущенного у ${side} — «${title}»: тега у версии нет`, `unreleased section in ${side} — “${title}”: the version has no tag`));
    }
  }
  if (out !== null) note(pick(lang, `записано: ${toPosix(path.relative(cwd, out))}`, `written: ${toPosix(path.relative(cwd, out))}`));
  for (const title of report.onlyOurs) note(pick(lang, `только у ours: ${title}`, `only in ours: ${title}`));
  for (const title of report.onlyTheirs) note(pick(lang, `только у theirs: ${title}`, `only in theirs: ${title}`));
  ...
  for (const what of report.onlyOursPlain) note(pick(lang, `только у ours, буллет без заголовка: ${what}`, `only in ours, bullet with no bold heading: ${what}`));
  for (const what of report.onlyTheirsPlain) note(pick(lang, `только у theirs, буллет без заголовка: ${what}`, `only in theirs, bullet with no bold heading: ${what}`));
  const section = `## ${report.section.ours}`;
  for (const { heading, subgroup, before } of report.placed) {
    if (subgroup !== null) note(pick(lang, `подгруппа ${subgroup} добавлена под ${heading ?? section}`, `subgroup ${subgroup} added under ${heading ?? section}`));
    else if (heading === null) note(pick(lang, `записи без заголовка добавлены в начало ${section}`, `entries without a heading added at the top of ${section}`));
    else if (before !== null) note(pick(lang, `заголовок ${heading} добавлен перед ${before}`, `heading ${heading} added before ${before}`));
    else note(pick(lang, `заголовок ${heading} добавлен в конец ${section}`, `heading ${heading} added at the end of ${section}`));
  }
  ...
  for (const what of report.duplicatesOurs) note(pick(lang, `повтор блока у ours, второе вхождение снято: ${what}`, `repeated block in ours, the second occurrence dropped: ${what}`));
  for (const what of report.duplicatesTheirs) note(pick(lang, `повтор блока у theirs, второе вхождение снято: ${what}`, `repeated block in theirs, the second occurrence dropped: ${what}`));
  for (const title of report.dropped) note(pick(lang, `снята относительно --base: ${title}`, `removed relative to --base: ${title}`));
