...
import { CliError, bad, git, gitCause, parseCommandArgs, toPosix, warn } from './util.js';
import { eolOf, readText, writeText } from './tasks.js';
...
function checkInvariants(merged, oursLines, theirsLines, report, lang) {
  ...
  // Граница — одно вхождение, под меткой конфликта два (столько кладёт `conflictEntry`); сумма по
  // сторонам недостижима, и проверка по ней была бы мёртвой.
...
export async function run(argv, { cwd }) {
  ...
  const out = values.out === undefined ? null : path.resolve(root, values.out);
  ...
  if (out !== null) note(tr(lang, `записано: ${toPosix(path.relative(root, out))}`, `written: ${toPosix(path.relative(root, out))}`));
