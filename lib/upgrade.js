...
// сокращает только хвост — migrate, init и выжимку CHANGELOG, — но не пробу; после migrate/init
// повторный обычный upgrade на ту же версию чинит оставленные живые пины.
import { spawnSync } from 'node:child_process';
import { readFileSync, writeFileSync } from 'node:fs';
import { CONFIG_FILE, gateEntry, loadProject, parseCli, pinRe, pinSep, saveConfig } from './config.js';
import { CliError, GIT_MAX_BUFFER, info, ok, parseCommandArgs, warn } from './util.js';
...
// Гейты, начинающиеся со старой команды, получают новую; остальные не трогаются. Запись с
// областью правится внутрь: `when` переживает перестановку пина. Нетронутая запись возвращается
// той же ссылкой — число заменённых считается сравнением с исходным списком.
export function rewriteGates(gates, oldCli, newCli) {
  return gates.map((gate) => {
    const { command, when } = gateEntry(gate);
    if (command !== oldCli && !command.startsWith(`${oldCli} `)) return gate;
    const next = newCli + command.slice(oldCli.length);
    return when === null ? next : { ...gate, command: next };
  });
}

// Пин живёт не только в конфиге: живая инструкция и конфигурация запуска могут звать снятую
// версию. `rewriteProsePins` и `lint` используют одно расширенное множество: живая
// markdown-проза, `package.json` и известные файлы CI. Исторические markdown-файлы остаются как были.
...
// --pin-only сначала меняет cli и gates, а migrate/init ставят новый штамп позже. Если затем
// обычный upgrade увидит ту же версию, ему всё равно нужно добраться до живых файлов, иначе
// оставленный старый пин уже никогда не будет переписан.
function hasStaleLivePins(root, docs, prefix, form) {
  if (!form || form.pin === null) return false;
  const re = pinRe(form);
  for (const [, abs] of livePinFiles(root, docs, prefix)) {
...
export async function run(argv, { cwd }) {
  ...
  if (current && compareVersions(target, current) === 0 && values.to === undefined && cfg.version === current && !hasStaleLivePins(root, cfg.docs, cfg.prefix, form)) {
  ...
  if (pinOnly) {
    info(tr(cfg.lang, `пин переставлен; дальше сам: ${newCli} migrate && ${newCli} init, затем ${newCli} upgrade для живых пинов`, `pin updated; finish manually: ${newCli} migrate && ${newCli} init, then ${newCli} upgrade for live pins`));
    return 0;
  }
