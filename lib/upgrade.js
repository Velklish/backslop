...
import { isJournalLine, livePinFiles, stalePins, symlinkComponent } from './mdwalk.js';
...
function hasStaleLivePins(root, cfg, form) {
  ...
  return stalePins(root, cfg.docs, cfg.prefix, form, (abs) => livePinText(root, abs).text ?? '').length > 0;
}
...
function exec(command, cwd, recovery, lang, { capture = false, timeout } = {}) {
  ...
  if (!shellPassed(r)) {
    ...
    throw new CliError(tr(lang, `шаг «${command}» — ${why}. ${recovery}`, `step “${command}” — ${why}. ${recovery}`));
  }
  return r.stdout;
}
...
export async function run(argv, { cwd, lang, timeout }) {
  ...
  if (values.to !== undefined && wanted === null) throw new CliError(tr(cfg.lang, `--to «${values.to}»: нужна форма X.Y.Z`, `--to “${values.to}”: expected X.Y.Z`));
  ...
  exec(from ? `${newCli} changelog --since v${from} --to v${target}` : `${newCli} changelog --to v${target}`, root, tr(cfg.lang, 'Обновление прошло, не напечаталась только выжимка CHANGELOG', 'Upgrade completed; only the CHANGELOG summary failed to print'), cfg.lang, { timeout });
