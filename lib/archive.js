...
import { CliError, git, info, ok, parseCommandArgs, today, toPosix } from './util.js';
...
function touchedDocs(root, cfg, task, range) {
  ...
  // заголовки неразличимы. Пути — `--relative`, от корня проекта, а не от toplevel git.
  const collect = (args, subjectRe = null) => {
    const r = git(root, ['log', '--format=%x01%s', '--name-only', '--relative', ...args, '--', ...pathspec]);
  ...
  collect([`--grep=${cfg.prefix}-`, 'HEAD'], subjectRe);
  return [...files].sort();
}
