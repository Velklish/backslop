...
export async function run(argv, { cwd, lang }) {
  ...
  if (!positionals[0]) throw new CliError(tr(cfg.lang, `нужен номер задачи: ${cfg.cli} archive <N> [--dry-run] [--range <база>..HEAD] | ${cfg.cli} archive <N.k> --into <M>`, `task number is required: ${cfg.cli} archive <N> [--dry-run] [--range <base>..HEAD] | ${cfg.cli} archive <N.k> --into <M>`));
  ...
  info(tr(cfg.lang, `переезд: ${oldRel} → ${newRel}`, `move: ${oldRel} → ${newRel}`));
  ...
  reportLinks(cfg, changed, dry, tr(cfg.lang, `archive: ${task.id} — файлов с поправленными ссылками ${changed.length}`, `archive: ${task.id} — files with updated links ${changed.length}`));
...
function archiveInto(project, tasks, task, rawInto, { dry, range }) {
  ...
  if (task.status !== 'minor') throw new CliError(tr(cfg.lang, `${task.id} не в minor/ (${task.rel}): --into закрывает пачкой только minor-записи; слияние карточек — ${cfg.cli} archive N с исходом «слита в …» в result.md`, `${task.id} is not in minor/ (${task.rel}): --into closes minor entries by batch only; merge cards with ${cfg.cli} archive N and a “merged into …” outcome in result.md`));
  ...
  if (batch.status !== 'archive') throw new CliError(tr(cfg.lang, `пачка ${batch.id} ещё в ${batch.status}/ — сначала закрой её: ${cfg.cli} archive ${batch.id}`, `batch ${batch.id} is still in ${batch.status}/ — close it first: ${cfg.cli} archive ${batch.id}`));
  ...
  const newRel = toPosix(path.relative(root, newFile));
  info(tr(cfg.lang, `переезд: ${task.rel} → ${newRel}`, `move: ${task.rel} → ${newRel}`));
  const changed = relocateTask(project, task, newRel, { dry });
  reportLinks(cfg, changed, dry, tr(cfg.lang, `archive: ${task.id} → пачка ${batch.id} — файлов с поправленными ссылками ${changed.length}`, `archive: ${task.id} → batch ${batch.id} — files with updated links ${changed.length}`));
  if (dry) info(dryRunLine(cfg));
  else info(tr(cfg.lang, `исход ${task.id} назови строкой в result.md пачки ${batch.id}; своего result.md у записи нет`, `name the outcome of ${task.id} in the result.md of batch ${batch.id}; the entry has no result.md of its own`));
  return 0;
}

function dryRunLine(cfg) {
  return tr(cfg.lang, '--dry-run: ничего не записано', '--dry-run: nothing was written');
}

// A dry run reports what the link rewrite would touch, without the success mark.
function reportLinks(cfg, changed, dry, done) {
  if (dry) info(tr(cfg.lang, `ссылки поправились бы в файлах: ${changed.length}`, `would update links in ${changed.length} files`));
  else ok(done);
