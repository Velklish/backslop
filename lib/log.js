...
export function namedResultOutcome(text, prefix, lang = 'ru') {
...
export function outcomeWordMissing(prefix, lang = 'ru') {
  return tr(lang,
    `result.md не называет исход словом словаря — выполнена, отклонена, снята с плана или слита в ${prefix}-N — ни в первом абзаце, ни в заголовке: голое «Закрыта» исходом не считается, свёртка прочла бы его «выполнена», а без слова поставила бы «—»`,
    `result.md names no outcome word — completed, rejected, or merged into ${prefix}-N — in its first paragraph or heading: a bare “Closed” is not an outcome, folding would read it as “completed”, and with no word at all it would write “—”`);
}
