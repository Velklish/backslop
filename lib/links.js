...
// from the root, anything else from `fromDir`; null when a `%`-escape is malformed.
export function normalizeHrefTarget(fromDir, target) {
  let decoded;
  try {
  ...
  return path.posix.normalize(decoded.startsWith('/') ? decoded.slice(1) : path.posix.join(fromDir, decoded));
}
...
export function brokenLinks(file, root = null) {
  const base = root ?? path.parse(file).root;
  const fromDir = toPosix(path.relative(base, path.dirname(file)));
  const listings = new Map();
  const out = [];
  for (const href of relativeLinks(readText(file))) {
    const { target } = splitHref(href);
    if (!target) continue;
    const rel = normalizeHrefTarget(fromDir, target);
    if (rel === null) {
      out.push({ href, real: null });
      continue;
    }
    const real = caseMismatch(base, rel, listings);
    if (real !== null) out.push({ href, real });
    else if (!existsSync(path.join(base, rel))) out.push({ href, real: null });
  }
  return out;
}
...
function resolveTarget(file, target, root) {
  const base = root ?? path.parse(file).root;
  const rel = normalizeHrefTarget(toPosix(path.relative(base, path.dirname(file))), target);
  return rel === null ? null : path.join(base, rel);
}
...
export function rewriteIncomingLinks(text, fileDir, oldPath, newPath) {
  return mapHrefs(text, (href) => {
    ...
    return `${rooted ? `/${newPath}` : path.posix.relative(fileDir, newPath)}${rest}`;
  });
}
