...
export function brokenLinks(file, root = null, prefix = '') {
...
function resolveTarget(file, target, root, prefix) {
  const base = root ?? path.parse(file).root;
  const rel = normalizeHrefTarget(toPosix(path.relative(base, path.dirname(file))), target, prefix);
...
export function directoryLinks(file, root = null, prefix = '') {
