...
// Инлайновые ссылки на существующий каталог — `{ text, href }`. Текст — от ближайшей `[` перед `]`,
// из исходника: незакрытая скобка раньше в абзаце не в счёт, а `` [`BS-5`](…) `` — номер.
export function directoryLinks(file, root = null) {
  const text = readFileSync(file, 'utf8');
  const out = [];
  for (const m of blankCode(text).matchAll(INLINE_LINK)) {
    const href = m[1] ?? m[2];
    const target = href.split(/[#?]/)[0];
...
// Обход ЦЕЛЕЙ ссылок текста: `fn` получает цель целиком — путь вместе с якорем — и возвращает
// новую цель либо null, если менять нечего. Нужен там, где якорь меняется вместе с путём:
// свёртка ведёт ссылку на строку журнала, и якорь исчезнувшего файла (`#контекст`) на ней не
// значит ничего. Ссылки внутри блоков кода тоже переписываются — разбирать markdown ради этого
...
// Ссылки соседей на СВЁРНУТЫЙ файл: `resolve` получает путь цели от корня репозитория и
// возвращает готовую цель — путь с якорем — либо null.
export function rewriteFoldedLinks(text, fileDir, resolve) {
  return mapHrefs(text, (href) => {
    const cut = href.search(/[#?]/);
    const target = cut === -1 ? href : href.slice(0, cut);
    if (!target) return null;
    return resolve(path.posix.normalize(path.posix.join(fileDir, target)), fileDir);
  });
}
