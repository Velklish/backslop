...
import { liveMarkdown, mdFiles } from './mdwalk.js';
...
import { TEMPLATES_DIR, templateParity } from './templates.js';
...
// Пин живёт и в прозе: живая инструкция зовёт версию текстом команды, и `upgrade` её
// переставляет вместе с конфигом. Расхождение — предупреждение, а не ошибка: команда со старым
// пином работает, просто зовёт не ту версию. Записи о моменте (CHANGELOG, ADR, архив) не
// считаются — их номер описывает историю. Голый `vX.Y.Z` в прозе тоже не считается: он законно
// называет историю, и проверка на него краснела бы на каждом таком предложении.
function lintProsePin({ root, cfg }, note) {
  const form = parseCli(cfg.cli);
  ...
  const re = pinRe(form);
  for (const [, abs] of liveMarkdown(root, cfg.docs, cfg.prefix)) {
    readText(abs).split('\n').forEach((line, i) => {
      for (const m of line.matchAll(re)) {
        if (m[1] === form.pin) continue;
        note(abs, tr(cfg.lang,
