...
import {
  FIELD_AREA, FIELD_CREATED, FIELD_DEPS, FIELD_ORDER, FIELD_PARENT, FIELD_TAKEN, SECTION_DEFERRED, canonicalId, fieldName, fieldOccurrences, getField, idMentionRe, orderOf, readText, readTitle, scanTasks,
...
// Ошибки красят гейт; предупреждения только печатаются: сведения о версии раскладки и находка
// под закрытым родителем требуют хода, но не блокируют проект.
...
function lintStatusFields({ root, cfg }, tasks, err) {
  ...
  for (const t of tasks) {
    ...
    for (const name of [FIELD_ORDER, FIELD_AREA, FIELD_CREATED, FIELD_TAKEN, FIELD_DEPS, FIELD_PARENT]) {
...
function isTodoPlaceholder(line) {
  ...
  if (field) value = field[1].trim();
  else if (/^[-*+]\s+/.test(value)) value = value.replace(/^[-*+]\s+/, '').trim();
  return TODO_VALUE.test(value);
}
