// Гейты трекера: то, что руками разъезжается молча, — битая ссылка, дубль номера, архив без
// результата… Отказ перечисляет всё найденное, код возврата — 1 при ошибках (03-lint.md).
import { existsSync, readFileSync, readdirSync } from 'node:fs';
import path from 'node:path';
import { CONFIG_FILE, SOURCE, STATUSES, gateEntry, loadProject, parseCli, pinRe, pinSep } from './config.js';
import { blankCode, blankFences, brokenLinks, directoryLinks, normalizeHrefTarget, relativeLinks, repoPrefix, splitHref } from './links.js';
import { LOG_FILE, brokenLogLines, hasNamedOutcome, logAnchor, logFile, outcomeWordMissing, readLogText } from './log.js';
import { UnreadableDir, mdFiles, resolvedEntry, rootMarkdown, stalePins } from './mdwalk.js';
import { ADR_FILE_RE, scanAdrs } from './adr.js';
import { formatId } from './ids.js';
import {
  FIELD_AREA, FIELD_COST, FIELD_CREATED, FIELD_DEPS, FIELD_ORDER, FIELD_PARENT, FIELD_PREV_ORDER, FIELD_TAKEN, SECTION_DEFERRED, SECTION_EVIDENCE, canonicalId, fieldName, fieldOccurrences, getField, hasResultTodo, hasTodoPlaceholder, idMentionRe, isPlaceholderLine, isTodoPlaceholder, orderOf, parseCost, readTitle, scanTasks,
  sectionBody, sectionName, sectionOccurrences, taskDirRe, taskFileRe,
} from './tasks.js';
import { splitLines } from './text.js';
import { CHANGELOG_ENTRY } from './changelog-format.js';
import {
  CliError, DATE_RE, bad, escapeRe, git, gitCause, lsFiles, ok, parseCommandArgs, readJson, readText, statOrNull, toPosix, warn,
} from './util.js';
...
import { livePinText } from './upgrade.js';
...
const PIN_IN_PROSE = new RegExp(`(?:${escapeRe(SOURCE)}(?:\\.git)?#v?|backslop@)(\\d+\\.\\d+\\.\\d+)`, 'g');
...
export function lintProject(project) {
  ...
  lintLinks(project, err, prefix);
  lintAdapters(project, err, prefix);
...
// 11. Релиз инструмента: версия пакета, штамп, секция CHANGELOG и пины в прозе — одно число.
// Только в репозитории самого backslop: у чужого проекта своей версии инструмента нет.
function lintReleaseVersions({ root, cfg }, err) {
  ...
  try {
    ...
    err(pkgFile, tr(cfg.lang, `не разбирается: ${e.cause.message}`, `cannot be parsed: ${e.cause.message}`));
    return;
  }
  ...
  if (existsSync(changelog) && !new RegExp(`^## v${escapeRe(version)}\\b`, 'm').test(readText(changelog))) {
...
// 14. Байт ниже 0x09 в исходнике: NUL прячет файл от git и grep, прочие не видны глазу. Только в
// репозитории инструмента: у потребителя docs/** и test/** законно несут бинарные файлы.
...
function lintControlBytes({ root, cfg }, err, note) {
  ...
  try {
    ...
    if (!(e instanceof CliError)) throw e;
    note(path.join(root, 'templates'), tr(cfg.lang, `непечатаемые байты не проверены: ${e.message}`, `non-printable bytes were not checked: ${e.message}`));
    return;
  }
  for (const rel of files) {
    ...
    const why = bytes[at] === 0
      ? tr(cfg.lang, 'NUL делает файл бинарным для git и grep, и поиск по нему молчит', 'NUL makes the file binary to git and grep, and searching it finds nothing')
      : tr(cfg.lang, 'невидимый управляющий байт: в редакторе и в выводе его не видно', 'an invisible control byte: neither an editor nor the output shows it');
    err(file, tr(cfg.lang,
      `байт ${code} на смещении ${at} (строка ${line}): ${why} — запиши его escape-последовательностью (\\u00${code.slice(2)})`,
      `byte ${code} at offset ${at} (line ${line}): ${why} — write it as an escape sequence (\\u00${code.slice(2)})`));
  }
}
...
function lintVersion({ root, cfg }, note) {
  ...
  if (!cfg.version) {
    ...
    if (pin !== null && pin !== cfg.version) note(file, tr(cfg.lang, `пин в cli v${pin} расходится со штампом v${cfg.version} — запусти ${cfg.cli} upgrade`, `cli pin v${pin} differs from version stamp v${cfg.version} — run ${cfg.cli} upgrade`));
  }
}

// Старый пин в живом файле — ошибка: команда с ним может звать снятую версию. Что живое, а что
// история, — docs/reference/03-lint.md, абзац «Пин сверяется с `cli`…».
function lintProsePin({ root, cfg }, err) {
  ...
  const re = pinRe(form);
  for (const { abs, lineNo, match } of stalePins(root, cfg.docs, cfg.prefix, form)) {
    const { text, link } = livePinText(root, abs);
    const why = link !== null ? tr(cfg.lang, `путь идёт через symlink ${link}`, `the path goes through the symlink ${link}`) : tr(cfg.lang, 'файл не в UTF-8', 'the file is not UTF-8');
    const fix = text !== null
      ? tr(cfg.lang, `переставит ${cfg.cli} upgrade`, `${cfg.cli} upgrade rewrites it`)
      : tr(cfg.lang, `upgrade его не переписывает (${why}) — поправь руками`, `upgrade does not rewrite it (${why}) — edit it by hand`);
    err(abs, tr(cfg.lang,
      `строка ${lineNo}: пин ${match[0]} расходится с cli — ожидается ${expected}; ${fix}`,
      `line ${lineNo}: pin ${match[0]} differs from cli — expected ${expected}; ${fix}`));
  }
  ...
  for (const [at, command] of commands) {
    for (const m of command.matchAll(re)) {
      ...
      err(path.join(root, CONFIG_FILE), tr(cfg.lang,
        `${at}: пин ${m[0]} расходится с cli — ожидается ${expected}; переставит ${cfg.cli} upgrade`,
        `${at}: pin ${m[0]} differs from cli — expected ${expected}; ${cfg.cli} upgrade rewrites it`));
    }
  }
}

// 1. Ссылки: docs/**, корневые *.md и скиллы backslop.
function lintLinks({ root, cfg, dirs }, err, prefix) {
  const files = [...mdFiles(dirs.docs, '')];
  files.push(...rootMarkdown(root, files));
  for (const [, abs] of files) {
    lintBrokenLinks(abs, root, cfg, err, prefix);
    lintDirectoryLinks(abs, root, cfg, err, prefix);
  }
}

function lintBrokenLinks(file, root, cfg, err, prefix) {
  for (const { href, real } of brokenLinks(file, root, prefix)) {
    if (real === null) err(file, tr(cfg.lang, `битая ссылка ${href}`, `broken link ${href}`));
    else err(file, tr(cfg.lang, `цель ссылки отличается регистром букв: ${real} (ссылка ${href})`, `link target differs in case: ${real} (link ${href})`));
  }
}

// Каталог — законная цель, но ссылка, чей текст называет задачу, обещает её карточку.
function lintDirectoryLinks(file, root, cfg, err, prefix) {
  const names = new RegExp(idMentionRe(cfg.prefix).source);
  for (const { text, href } of directoryLinks(file, root, prefix)) {
    if (names.test(text)) err(file, tr(cfg.lang, `ссылка [${text}](${href}) ведёт на каталог, а её текст называет задачу — веди на файл задачи или строку журнала`, `link [${text}](${href}) points to a directory while its text names a task — point it at the task file or its journal line`));
  }
}
...
function lintAdapters({ root, cfg }, err, prefix) {
  ...
  for (const [, file] of generatedAdapterFiles(root, cfg)) {
    lintBrokenLinks(file, root, cfg, err, prefix);
    lintDirectoryLinks(file, root, cfg, err, prefix);
  }
}

// 2. Номера: уникальны, совпадают с заголовком, у sub-ID есть родитель;
// чужие файлы в каталогах статусов.
function lintTaskFiles({ cfg, dirs }, tasks, err) {
  ...
  for (const t of tasks) {
    if (t.sub !== null && !tasks.some((p) => p.num === t.num && p.sub === null)) {
      err(t.file, tr(cfg.lang, `находка ${t.id} без родителя ${formatId(cfg.prefix, t.num)}`, `finding ${t.id} has no parent ${formatId(cfg.prefix, t.num)}`));
    }
  }
...
function lintBacklogLayout({ root, cfg, dirs }, err) {
  if (!existsSync(dirs.backlog)) {
    err(dirs.backlog, tr(cfg.lang, `каталога бэклога нет — ${cfg.cli} init`, `backlog directory is missing — run ${cfg.cli} init`));
    return;
  }
  for (const e of readdirSync(dirs.backlog, { withFileTypes: true })) {
    ...
    if (resolvedEntry(root, dirs.backlog, e).isDirectory()) {
      ...
      err(abs, tr(cfg.lang, 'файл, а нужен каталог', 'a file, expected a directory'));
...
// Заглушка — всё значение строки, а не подстрока объяснения; фенсы — примеры. `triage/` не
// проверяется, «Область» в `minor/` — предупреждение гейта 4 (docs/reference/03-lint.md, гейт 4).
  ...
  for (const t of tasks) {
    ...
    if (t.status === 'queue') {
      ...
      if (rank === null) err(t.file, tr(cfg.lang, `в очереди без поля «${fieldName(FIELD_ORDER, cfg.lang)}» — место в очереди не задано`, `queue task has no “${fieldName(FIELD_ORDER, cfg.lang)}” field — queue position is not set`));
        ...
        err(t.file, tr(cfg.lang, `«${fieldName(FIELD_ORDER, cfg.lang)}» ${rank} уже у ${first} — переставь: ${cfg.cli} mv N queue --top | --after M`, `“${fieldName(FIELD_ORDER, cfg.lang)}” ${rank} is already used by ${first} — reorder with ${cfg.cli} mv N queue --top | --after M`));
      } else ranked.set(rank, t.file);
    }
    if (t.status === 'active') {
      const taken = getField(text, FIELD_TAKEN);
      if (!taken || !DATE_RE.test(taken)) err(t.file, tr(cfg.lang, `в работе без даты «${fieldName(FIELD_TAKEN, cfg.lang)}: ГГГГ-ММ-ДД»`, `active task has no “${fieldName(FIELD_TAKEN, cfg.lang)}: YYYY-MM-DD” date`));
    }
    if (t.status === 'minor') {
      ...
      else if (parsed.level !== 'minor' && !parsed.hypothesis) err(t.file, tr(cfg.lang, `«${label}» ${parsed.level} без пометки «гипотеза»: с уликой такая находка чинится сейчас, а не ждёт пачки — чини или ${cfg.cli} mv N.k triage`, `“${label}” ${parsed.level} without the “hypothesis” mark: with evidence such a finding is fixed now, not queued for a batch — fix it or ${cfg.cli} mv N.k triage`));
      const evidence = sectionBody(text, SECTION_EVIDENCE);
      const heading = sectionName(SECTION_EVIDENCE, cfg.lang);
      if (!evidence) err(t.file, tr(cfg.lang, `в minor/ без раздела «## ${heading}» или он пуст: запись уезжает в пачку без разбора — впиши улику: путь со строкой, команда с выводом и кодом или замер числом`, `minor entry has no “## ${heading}” section or it is empty: the entry goes to a batch without review — write the evidence: a path with a line, a command with its output and exit code, or a measurement with a number`));
      else if (hasTodoPlaceholder(evidence)) err(t.file, tr(cfg.lang, `раздел «${heading}» не заполнен: осталась заглушка [TODO]`, `“${heading}” section is incomplete: [TODO] remains`));
    }
...
// 4. «Область» разобранной задачи вписана; заглушка `[TODO]` от `new` — незаполненное поле.
// `triage/` и архив не проверяются, в `minor/` это предупреждение: область там ставит approver.
...
// 5. Архив: каталог на задачу с task.md и дописанным result.md. Каталог законен сколько угодно
// долго, свёрнутую задачу сторожит гейт 13 (ADR-026).
function lintArchive({ root, cfg, dirs }, err) {
  if (!existsSync(dirs.archive)) return;
  if (!statOrNull(dirs.archive)?.isDirectory()) {
    err(dirs.archive, tr(cfg.lang, 'файл, а нужен каталог', 'a file, expected a directory'));
    return;
  }
  ...
  for (const e of readdirSync(dirs.archive, { withFileTypes: true })) {
    ...
    if (!statOrNull(minorDir)?.isDirectory()) {
      err(minorDir, tr(cfg.lang, 'файл, а нужен каталог', 'a file, expected a directory'));
      continue;
    }
    const fileRe = taskFileRe(cfg.prefix);
...
// 13. Журнал закрытых: запись разбирается, якорь равен номеру, ревизия достижима из HEAD, ссылка
// `LOG.md#<якорь>` ведёт на строку — единственная проверка якоря в lint (03-lint.md, гейт 13).
...
function lintAdrIndex({ root, cfg, dirs }, err, prefix) {
  ...
  if (existsSync(dirs.adr) && !statOrNull(dirs.adr)?.isDirectory()) err(dirs.adr, tr(cfg.lang, 'файл, а нужен каталог', 'a file, expected a directory'));
  ...
  for (const a of adrs) {
    if (!linked.has(toPosix(path.relative(root, a.file)))) err(a.file, tr(cfg.lang, `нет строки в ${path.basename(dirs.docsReadme)} — таблица ADR ведётся вручную`, `no row in ${path.basename(dirs.docsReadme)} — the ADR table is maintained manually`));
  }
}
...
// 10. Цитата живого файла: блок `<!-- quote:<путь> -->…<!-- /quote -->` обязан найтись в названном
// файле; `docs/archive/` — снимок момента и не проверяется (docs/reference/03-lint.md, гейт 10).
  ...
  for (const [, abs] of files) {
    ...
    for (const block of quoteBlocks(text)) {
      if (block.stray !== undefined) {
        err(abs, tr(cfg.lang, `строка ${block.stray + 1}: «/quote» не закрывает ни одного блока цитаты`, `line ${block.stray + 1}: “/quote” closes no quote block`));
        continue;
      }
      if (block.malformed !== undefined) {
        err(abs, tr(cfg.lang, `строка ${block.malformed + 1}: маркер цитаты не разбирается`, `line ${block.malformed + 1}: the quote marker does not parse`));
        continue;
      }
      if (block.body === null) {
...
// Цитата — непрерывный кусок файла; пробелы по краям строк сняты с обеих сторон. Отбрасывается
// только обёртка-фенс вокруг цитаты, фенсы внутри остаются (docs/reference/03-lint.md, гейт 10).
...
export async function run(argv, { cwd, lang }) {
  ...
  let report;
  try {
    report = lintProject(project);
  } catch (e) {
    if (!(e instanceof UnreadableDir)) throw e;
    const rel = toPosix(path.relative(project.root, e.dir)) || '.';
    throw new CliError(tr(project.cfg.lang,
      `${rel}: каталог не читается (${e.code}) — lint его не обходит; верни права на чтение или вынеси каталог из проекта`,
      `${rel}: the directory is not readable (${e.code}) — lint cannot walk it; restore read access or move it out of the project`));
  }
