...
import { blankFences, brokenLinks, directoryLinks, normalizeHrefTarget, relativeLinks, splitHref } from './links.js';
import { LOG_FILE, brokenLogLines, logAnchor, logFile, namedResultOutcome, outcomeWordMissing, readLogText } from './log.js';
import { liveMarkdown, livePinFiles, mdFiles, resolvedEntry, rootMarkdown } from './mdwalk.js';
...
function lintLinks({ root, cfg, dirs }, err) {
  ...
  for (const [, abs] of files) {
    lintBrokenLinks(abs, root, cfg, err);
    lintDirectoryLinks(abs, root, cfg, err);
  }
}

function lintBrokenLinks(file, root, cfg, err) {
  for (const { href, real } of brokenLinks(file, root)) {
...
function lintDirectoryLinks(file, root, cfg, err) {
  const names = new RegExp(idMentionRe(cfg.prefix).source);
  for (const { text, href } of directoryLinks(file, root)) {
...
function lintAdapters({ root, cfg }, err) {
  ...
  for (const [, file] of generatedAdapterFiles(root, cfg)) {
    lintBrokenLinks(file, root, cfg, err);
    lintDirectoryLinks(file, root, cfg, err);
  }
}
...
function lintLog({ root, cfg, dirs }, tasks, err, note) {
  ...
  for (const [rel, abs] of files) {
    ...
    for (const href of relativeLinks(readText(abs))) {
      ...
      if (normalizeHrefTarget(dir, target) !== logRel || anchors.has(anchor)) continue;
      err(abs, tr(cfg.lang,
...
function lintAdrIndex({ root, cfg, dirs }, err) {
  ...
  const linked = new Set(relativeLinks(readText(dirs.docsReadme))
    .map((href) => normalizeHrefTarget(readmeDir, splitHref(href).target)));
  for (const a of adrs) {
...
// Блоки цитат: маркеры — по тексту с погашенными фенсами, тело — из сырых строк (цитата кода сама
// обёрнута фенсом). Незакрытый блок отдаётся с body === null.
