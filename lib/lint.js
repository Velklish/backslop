...
import {
  FIELD_ORDER, FIELD_TAKEN, SECTION_DEFERRED, fieldName, getField, idMentionRe, orderOf, readText, readTitle, scanTasks,
...
function lintTaskFiles({ cfg, dirs }, tasks, err) {
  ...
  for (const t of tasks) {
    ...
    else if (title.id !== t.id) err(t.file, tr(cfg.lang, `заголовок называет ${title.id}, имя файла — ${t.id}`, `heading names ${title.id}, but filename names ${t.id}`));
  }
  for (const t of tasks) {
