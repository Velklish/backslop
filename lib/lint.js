...
import {
  CliError, DATE_RE, bad, escapeRe, git, gitCause, lsFiles, ok, parseCommandArgs, readJson, readText, realpathOrNull, statOrNull, toPosix, warn,
} from './util.js';
...
// Репозиторий инструмента — тождество `templates/` проекта с каталогом запущенного CLI: не гаснет
// от переименований и не задевает чужой `templates/`. По realpath — иначе symlink выключил бы гейт.
export function sameDir(a, b) {
  const ra = realpathOrNull(a);
  return ra !== null && ra === realpathOrNull(b);
}
