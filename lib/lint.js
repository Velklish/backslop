...
function lintVersion({ root, cfg }, note) {
  ...
  if (form && pin === null) note(file, `cli без пина тянет HEAD ветки при каждом запуске — запусти ${cfg.cli} upgrade`);
  if (!cfg.version) {
    note(file, `нет штампа версии — запусти ${cfg.cli} upgrade или init`);
  } else {
    const cmp = compareVersions(cfg.version, TOOL_VERSION);
    if (cmp < 0) note(file, `скелет старее инструмента: v${cfg.version} < v${TOOL_VERSION} — запусти ${cfg.cli} upgrade`);
    if (cmp > 0) note(file, `штамп новее инструмента: v${cfg.version} > v${TOOL_VERSION} — обнови установку или пин в cli`);
    if (pin !== null && pin !== cfg.version) note(file, `пин в cli v${pin} расходится со штампом v${cfg.version} — запусти upgrade`);
  }
}
...
export async function run(argv, { cwd }) {
  ...
  if (errors.length) {
    bad(`lint: ошибок ${errors.length}${tail}`);
    return 1;
  }
  ok(`lint: ошибок нет${tail}`);
  return 0;
}
