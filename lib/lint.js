...
import { LOG_FILE, brokenLogLines, logAnchor, logFile, namedResultOutcome, outcomeWordMissing, readLogText } from './log.js';
import { isJournalLine, liveMarkdown, livePinFiles, mdFiles, resolvedEntry, rootMarkdown } from './mdwalk.js';
import { ADR_FILE_RE, scanAdrs } from './adr.js';
import {
  FIELD_AREA, FIELD_COST, FIELD_CREATED, FIELD_DEPS, FIELD_ORDER, FIELD_PARENT, FIELD_PREV_ORDER, FIELD_TAKEN, SECTION_DEFERRED, SECTION_EVIDENCE, canonicalId, fieldName, fieldOccurrences, getField, hasResultTodo, idMentionRe, orderOf, parseCost, readText, readTitle, scanTasks,
  sectionBody, sectionName, sectionOccurrences, splitLines, taskDirRe, taskFileRe,
} from './tasks.js';
import { bad, git, gitCause, ok, parseCommandArgs, toPosix, warn } from './util.js';
...
// Маркер списка снимается до разбора, а имя поля не начинается с `[TODO`: иначе заглушка
// `- [TODO: текст]` читалась бы полем «[TODO» и проходила гейт молча.
const TODO_FIELD = /^(?:\*\*[^*\n]+:\*\*|\*\*[^*\n]+\*\*:|(?!\[TODO)[^:\n]+:)[ \t]*(.*)$/;
...
const PIN_IN_PROSE = new RegExp(`(?:${SOURCE.replace(/[.\\]/g, '\\$&')}(?:\\.git)?#v?|backslop@)(\\d+\\.\\d+\\.\\d+)`, 'g');
...
function lintControlBytes({ root, cfg }, err, note) {
  if (!sameDir(path.join(root, 'templates'), TEMPLATES_DIR)) return;
  const ls = git(root, ['ls-files', '-z', '--', ...SOURCE_TREES]);
  if (ls.status !== 0) {
    note(path.join(root, 'templates'), tr(cfg.lang, `непечатаемые байты не проверены: git ls-files — ${gitCause(ls, cfg.lang)}`, `non-printable bytes were not checked: git ls-files — ${gitCause(ls, cfg.lang)}`));
    return;
  }
...
function lintProsePin({ root, cfg }, err) {
  ...
  for (const [rel, abs] of livePinFiles(root, cfg.docs, cfg.prefix)) {
    ...
    readText(abs).split('\n').forEach((line, i) => {
      if (isJournalLine(rel, cfg.docs, line)) return;
      for (const m of line.matchAll(re)) {
        if (m[1] === form.pin) continue;
        err(abs, tr(cfg.lang,
          `строка ${i + 1}: пин ${m[0]} расходится с cli — ожидается ${expected}; ${fix}`,
          `line ${i + 1}: pin ${m[0]} differs from cli — expected ${expected}; ${fix}`));
      }
    });
  }
...
function lintAdapters({ root, cfg }, err, prefix) {
  ...
  for (const tool of cfg.tools) {
    for (const file of owned[tool]) {
      ...
      // Каталог на owned-пути: init на нём отказывает, а предикат владения legacy-пути на тип
      // записи не смотрит — без этой ветви гейт был бы зелёным там, где init не работает.
...
// Gate 4 also reads a numbered item, a task box and each table cell; `mv` keeps the line meaning.
...
function lintArchive({ root, cfg, dirs }, err) {
  ...
  for (const e of readdirSync(dirs.archive, { withFileTypes: true })) {
    ...
    else if (!namedResultOutcome(readText(result), cfg.prefix)) err(abs, outcomeWordMissing(cfg.prefix, cfg.lang));
...
export function lintChangelog(root, err, lang = 'ru') {
  ...
  blankFences(readText(file)).split('\n').forEach((raw, i) => {
