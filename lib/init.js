...
import { TEMPLATES_DIR, isToolRepo, probeRule, renderProjectTemplate, templateRel } from './templates.js';
...
export async function run(argv, { cwd, lang }) {
  ...
  if (values.dir !== undefined && !isProjectPath(values.dir)) {
    throw new CliError(pick(knownLang, `--dir «${values.dir}»: нужен относительный путь внутри проекта`, `--dir “${values.dir}”: expected a relative path inside the project`));
  }
  ...
  if (existsSync(configFile)) {
    ...
    if (!PREFIX_RE.test(prefix)) throw new CliError(pick(knownLang, `--prefix «${prefix}»: нужны 2–6 заглавных латинских букв или цифр, первая буква`, `--prefix “${prefix}”: expected 2–6 uppercase Latin letters or digits, starting with a letter`));
    const docs = dir ?? base.docs;
    const cli = values.cli ?? base.cli;
    if (values.cli !== undefined && !values.cli.trim()) throw new CliError(pick(knownLang, '--cli — непустая строка команды', '--cli must be a non-empty command string'));
  ...
  ok(tr(cfg.lang,
    `init: ${cfg.docs}/ (префикс ${cfg.prefix}), создано файлов ${created.length}, оставлено как есть ${skipped.length}`,
    `init: ${cfg.docs}/ (prefix ${cfg.prefix}), files created ${created.length}, left unchanged ${skipped.length}`));
  const outputs = cfg.tools.map((tool) => `${tool} ${rendered.counts[tool]}`).join(', ');
  info(tr(cfg.lang, `adapter outputs: ${outputs || 'нет'}`, `adapter outputs: ${outputs || 'none'}`));
  ...
  info(cfg.tools.length
    ...
    : isToolRepo(root) ? tr(cfg.lang,
      `дальше: ${cfg.cli} lint — проверка скелета; обновление — ${cfg.cli} upgrade`,
      `next: ${cfg.cli} lint validates the layout; update with ${cfg.cli} upgrade`)
    : tr(cfg.lang,
      `дальше: ${cfg.cli} lint — проверка скелета; скиллы, и с ними наполнение docs, ставит adapter: ${cfg.cli} init --tools claude|cursor|codex; обновление — ${cfg.cli} upgrade`,
      `next: ${cfg.cli} lint validates the layout; an adapter installs the skills, including the one that populates docs: ${cfg.cli} init --tools claude|cursor|codex; update with ${cfg.cli} upgrade`));
  return 0;
}
...
function parseTools(raw, lang) {
  ...
  if (!tools.length || !validTools(tools)) {
    throw new CliError(pick(lang, `--tools «${raw}»: claude, cursor, codex через запятую или none`, `--tools “${raw}”: a comma-separated list of claude, cursor, codex, or none`));
  }
