...
export async function run(argv, { cwd }) {
  ...
  if (existsSync(configFile)) {
    ...
    // Первый init loadConfig не зовёт: без этой проверки метка из `--dir` или `--cli` разорвала бы
    // блок, и повторный init такой проект уже не починил бы — он отказывал бы на конфиге.
