...
import {
  BLOCK_END, BLOCK_MARKERS, BLOCK_START, CONFIG_FILE, LANGS, PREFIX_RE, STATUSES, TOOLS, defaultCli, defaults, findRoot, layout, loadConfig, parseCli, saveConfig, validateBlockValue,
} from './config.js';
import { CLAUDE_STUB, checkAdapterRoots, cleanupAdapters, ensureClaudeStub, renderAdapters } from './adapters.js';
import { adapterRootRel } from './adapters-registry.js';
...
export async function run(argv, { cwd }) {
  ...
  if (existsSync(configFile)) {
    ...
    // Первый init строит конфиг сам и loadConfig не зовёт: без этих проверок метка из `--dir`
    // или `--cli` разорвала бы блок уже на первом запуске, а починить проект повторным init
    // было бы нечем — он читал бы конфиг и отказывал. Оба поля уезжают в блок, поэтому оба
    // идут через ту же проверку, что и в loadConfig.
    validateBlockValue(docs, 'docs', requestedLang ?? base.lang);
    validateBlockValue(cli, 'cli', requestedLang ?? base.lang);
    cfg = {
  ...
  // Проверка корней выбранных adapter'ов идёт до первой записи: отказ на symlink после
  // saveConfig оставлял проект с конфигом и скелетом docs, который тем же init уже не чинится.
  ...
  if (!cfg.probe) {
    notes.push(tr(cfg.lang,
      `probe в ${CONFIG_FILE} не объявлен: требования мутационной пробы в блоке нет — объяви команду полем probe или опиши пробу своим разделом вне блока`,
      `probe is not declared in ${CONFIG_FILE}: the block carries no mutation-probe requirement — declare the command in probe or describe the probe in a section of your own outside the block`));
  }
  ...
  // Adapter outputs — генерируемое: выбранные переписываются, снятые чистятся — и то и другое
  // только по предикату владения (маркер или legacy-путь). Чужие файлы в harness-каталогах
  // остаются, и те из них, что лежат на owned-путях, названы предупреждением.
  ...
  const ignoreState = upsertIgnore(root, cfg);
...
// Owned outputs — локальный generated результат, в git им делать нечего: три проекта на
// backslop завели три разных `.gitignore` руками, один держал правило вне git вовсе.
// Строки — по факту выбранных adapters; `/CLAUDE.md` только когда на диске лежит наш stub,
// иначе блок спрятал бы от git пользовательский файл.
...
function upsertIgnore(root, cfg) {
  const file = path.join(root, '.gitignore');
  const lines = ignoreLines(root, cfg);
  const present = existsSync(file) && readFileSync(file, 'utf8').includes(IGNORE_MARKERS[0]);
  if (!lines.length && !present) return tr(cfg.lang, 'не нужен', 'not needed');
  ...
  return upsertBlock(file, block, cfg.lang, IGNORE_MARKERS);
}
...
export function upsertBlock(file, block, lang = 'ru', [BLOCK_OPEN, BLOCK_CLOSE] = [BLOCK_START, BLOCK_END]) {
  if (!existsSync(file)) {
  ...
  if (start !== -1 && end !== -1 && end > start) {
    const next = text.slice(0, start) + block + text.slice(end + BLOCK_CLOSE.length);
    if (next !== text) writeFileSync(file, next);
    return next === text ? tr(lang, 'без изменений', 'unchanged') : tr(lang, 'обновлён', 'updated');
  }
  if (start !== -1 || end !== -1) throw new CliError(tr(lang, `${path.basename(file)}: маркер блока backslop без пары — поправь руками`, `${path.basename(file)}: unmatched backslop block marker — fix it manually`));
