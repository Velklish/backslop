...
export async function run(argv, { cwd }) {
  ...
  // Проверка корней harness идёт до первой записи: отказ на symlink после saveConfig оставлял
  // проект с конфигом и скелетом docs, который тем же init уже не чинится.
  checkAdapterRoots(root, cfg.lang);
  if (freshConfig) {
    saveConfig(root, cfg);
