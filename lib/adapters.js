...
function ownedPath(root, file, lang) {
  ...
  if (link !== null) throw new CliError(tr(lang, `adapter path содержит symlink: ${link}`, `adapter path contains a symlink: ${link}`));
  return target;
}

// Корни выбранных adapter'ов — до первой записи `init` (ADR-016): туда backslop пишет и оттуда
// снимает, и symlink там — отказ, куда бы ссылка ни вела. Корни невыбранных не проверяются:
// backslop в них не ходит, а общий harness-каталог ссылкой у проекта без этого adapter'а —
// законная раскладка. Отказ после записи конфига оставил бы проект, который тем же `init` не
// чинится.
export function checkAdapterRoots(root, tools, lang) {
  for (const tool of tools) {
    try {
      ownedPath(root, path.join(root, ...ADAPTER_ROOTS[tool]), lang);
    } catch (e) {
      if (!(e instanceof CliError)) throw e;
      throw new CliError(tr(lang,
        `${e.message} — сквозь чужую ссылку backslop не пишет и не снимает файлы. Замени корень harness обычным каталогом или сними adapter ${tool}: --tools без него, либо --tools none, если других adapters нет`,
        `${e.message} — backslop neither writes nor removes files through a foreign link. Replace the harness root with a plain directory or deselect the ${tool} adapter: --tools without it, or --tools none if no other adapters are selected`));
    }
  }
}
...
// Шаблоны кавычат `description`: плоский скаляр с «: » внутри как YAML-мэппинг не разбирается,
// и гейт фронтматтера у потребителя краснел на каждом `init` (BS-63). Кавычки снимаются здесь
// — Cursor кавычит значение заново сам (`JSON.stringify` ниже), и без снятия в `.mdc` уехало бы
// двойное экранирование. Форма закавыченного значения — JSON-строка, её держит гейт `npm test`.
function splitFrontmatter(text) {
  const m = text.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n/);
  if (!m) return { description: '', body: text };
  const value = m[1].split(/\r?\n/).find((line) => line.startsWith('description: '))?.slice(13) ?? '';
  return { description: value.startsWith('"') ? JSON.parse(value) : value, body: text.slice(m[0].length) };
}
...
export function cleanupAdapters(root, cfg) {
  ...
  for (const tool of TOOLS) {
    ...
    // Невыбранный adapter за ссылкой не снимается (ADR-016): сквозь чужую ссылку backslop не
    // читает и не пишет, а отказ из-за каталога, в который он не ходит, закрывал бы init навсегда.
    if (!selected && symlinkComponent(path.resolve(root), path.resolve(path.join(root, ...ADAPTER_ROOTS[tool]))) !== null) continue;
