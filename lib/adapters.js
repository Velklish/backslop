...
import {
  GENERATED_MARKER, adapterRel, cursorRel, hasGeneratedMarker, isOwnedAdapterFile, markGenerated,
...
// вёл; корни невыбранных не проверяются (ADR-016).
...
function splitFrontmatter(text, template, lang) {
  const m = text.match(FRONTMATTER);
  if (!m) return { description: '', body: text };
  const description = frontmatterField(text, 'description');
  if (description === null) {
    throw new CliError(tr(lang,
      `шаблон ${template}: description во фронтматтере — не JSON-строка`,
      `template ${template}: the frontmatter description is not a valid JSON string`));
  }
...
function cursorOutput(rendered, sourceRel, lang) {
  if (!sourceRel.endsWith('/SKILL.md')) return rewriteCursorLinks(rendered, sourceRel);
  const { description, body } = splitFrontmatter(rendered, `templates/${templateRel(lang, `skills/${sourceRel}`)}`, lang);
...
// предупреждением (ADR-015). Возвращает счёт записанного по adapter'ам и текст предупреждения.
export function renderAdapters(root, cfg, vars) {
  ...
  for (const tool of cfg.tools) {
    for (const [rel] of files) {
      ...
      const text = tool === 'cursor' ? cursorOutput(rendered, rel, cfg.lang) : rendered;
...
// candidate; `isOwnedAdapterFile` decides the removal (ADR-006).
