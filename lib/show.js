...
import { canonicalId, findTask, parseId, readTitle, scanTasks, taskFileRe } from './tasks.js';
...
export async function run(argv, { cwd, lang }) {
  ...
  if (sections.length) {
    ...
    if (attachments.length) {
      ...
      note(tr(cfg.lang, `вложения названы путём: git show ${rev}:./<путь> печатает файл`, `attachments are listed by path: git show ${rev}:./<path> prints one`));
    }
