...
export async function run(argv, { cwd }) {
  ...
  if (files.length) {
    ...
    for (const file of files) {
      // Форма `<rev>:./<путь>` — от текущего каталога, как у `cat-file` в `lib/fold.js`: без
      // `./` git резолвит путь от корня РАБОЧЕГО ДЕРЕВА, а `ls-tree` напечатал его от корня
      // проекта. В проекте-подкаталоге они расходятся на префикс, каждое чтение отказывает, и
      // команда сваливалась в ветку «тела файлом нет» с объяснением, которое неверно.
