...
import { findTask, parseId, readText, readTitle, scanTasks, taskFileRe, writeText } from './tasks.js';
...
function prepare(project, task, { single }) {
  ...
  if (TODO.test(resultText)) throw new CliError(tr(cfg.lang, `${dirRel}/result.md остался заглушкой: в нём [TODO] — допиши исход и проверки`, `${dirRel}/result.md is still a stub: it contains [TODO] — write the outcome and the verification`));
