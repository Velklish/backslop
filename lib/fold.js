...
import { appendLogLines, dateFromResult, formatLogLine, logAnchor, logFile, namedResultOutcome, outcomeFromResult, outcomeWordMissing, renderOutcome } from './log.js';
...
import { CliError, bad, git, gitCause, info, parseCommandArgs, toPosix, today, warn } from './util.js';
...
function draftNoteOne(cfg, entry) {
  return entry.rev !== null
    ? tr(cfg.lang,
      `заготовка сообщения коммита — в stdout; коммитить её не обязательно: тело уже в истории — ревизия записана в строку журнала, и ${cfg.cli} show ${entry.task.id} достаёт его оттуда. Squash или reset --soft через эту ревизию выбросит её, и тогда без заготовки тело задачи потеряется`,
      `the commit message draft is on stdout; committing it is optional: the body is already in history — the revision is recorded in the journal line, and ${cfg.cli} show ${entry.task.id} reads it from there. A squash or reset --soft across that revision drops it, and then without the draft the task body is lost`)
    : tr(cfg.lang,
...
function draftNoteMany(cfg, entries, embedded, dropped) {
  if (embedded.length) {
    return tr(cfg.lang,
      'заготовка сообщения коммита — в stdout, и в ней тела задач, которых нет в истории: закоммить свёртку вместе с ней, иначе эти тела потеряются',
      'the commit message draft is on stdout and carries the bodies of tasks that are not in history: commit the fold together with it, otherwise those bodies are lost');
  }
  if (!dropped.length) {
    return tr(cfg.lang,
...
function pickAll(project, tasks, olderThan) {
  return tasks
    ...
    .filter((t) => olderThan === null || closedAt(project, t) < olderThan);
}
...
function closedAt({ root, cfg }, task) {
  ...
  const minors = minorEntries(cfg, task.dir).map((e) => `${dirRel}/minor/${path.basename(e.file)}`);
...
function prepare(project, task, { single }) {
  ...
  // Одиночная свёртка — ход приёмки, и она идёт раньше lint: слово исхода требуется здесь, как у гейта 5.
  if (single && !namedResultOutcome(resultText, cfg.prefix)) throw new CliError(`${dirRel}/${outcomeWordMissing(cfg.prefix, cfg.lang)}`);
  ...
  const { rev, why, detail, at, cause } = bodyRev(root, dirRel, [`${dirRel}/task.md`, `${dirRel}/result.md`, ...minors.map((e) => e.rel)], cfg.lang);
...
function bodyRev(root, dirRel, bodyPaths, lang) {
  ...
  if (disk.status !== 0) {
    const cause = gitCause(disk, lang).split('\n')[0];
