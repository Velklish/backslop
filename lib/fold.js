...
import { appendLogLines, bodySection, dateFromResult, formatLogLine, logAnchor, logFile, namedResultOutcome, outcomeFromResult, outcomeWordMissing, renderOutcome } from './log.js';
...
export async function run(argv, { cwd }) {
  ...
  const entries = picked.map((t) => prepare(project, t, { single, embed }));
...
function prepare(project, task, { single, embed }) {
  const { root, cfg, dirs } = project;
  const dirRel = toPosix(path.relative(root, task.dir));
...
function unsavedAttachments(root, rels, lang) {
  ...
  const { moved, cause } = kept.length ? drift(root, blobs, kept, lang) : { moved: [] };
...
function bodyRev(root, dirRel, bodyPaths, lang) {
  ...
  const { moved, cause } = drift(root, blobs, [...blobs.keys()], lang);
  if (cause) return { rev: null, why: DIVERGED, detail: dirRel, at: rev, cause };
  return moved.length ? { rev: null, why: DIVERGED, detail: moved[0], at: rev, cause: null } : { rev, why: null, detail: '' };
}
...
function drift(root, blobs, rels, lang) {
  const disk = git(root, ['hash-object', '--', ...rels]);
  if (disk.status !== 0) return { moved: [], cause: gitCause(disk, lang).split('\n')[0] };
  const ids = disk.stdout.trim().split('\n');
  return { moved: rels.filter((rel, i) => ids[i] !== blobs.get(rel)), cause: null };
}
