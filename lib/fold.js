...
import { findTask, hasResultTodo, parseId, readTitle, scanTasks } from './tasks.js';
...
export async function run(argv, { cwd, lang }) {
  const { values, positionals } = parseCommandArgs(argv, { 'dry-run': { type: 'boolean' }, 'older-than': { type: 'string' }, 'embed-missing': { type: 'boolean' } }, {
    positionals: 1,
    lang,
    tooMany: ['номер задачи один: массовая свёртка — та же команда без номера', 'a single task number is expected: bulk folding is the same command without a number'],
  });
  ...
  if (!picked.length) {
    note(tr(cfg.lang, 'сворачивать нечего: несвёрнутых каталогов в архиве нет', 'nothing to fold: the archive has no unfolded directories'));
    return 0;
  }
  ...
  if (dry) {
    note(tr(cfg.lang, `--dry-run: свернулось бы задач ${entries.length}, ничего не записано`, `--dry-run: ${entries.length} tasks would be folded, nothing was written`));
    return 0;
  }
...
function removeDirs({ root, cfg }, entries) {
  ...
  if (tracked.length) {
    ...
    if (r.status !== 0) {
      throw new CliError(tr(cfg.lang,
        `git rm отказал: ${gitCause(r, cfg.lang)} — каталоги задач не удалены, журнал не тронут; разбери дерево и повтори`,
        `git rm failed: ${gitCause(r, cfg.lang)} — task directories were not removed, the journal is untouched; sort the tree out and retry`));
    }
  }
