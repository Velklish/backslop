...
The subject is one task from taking it to archiving it. Tracker rules are in `{{docs}}/backlog/README.md`; this file lays out the order by role and step. `{{cli}}` is the command recorded in `backslop.json`; below it is simply called `backslop`.
...
4. **Run gates before reporting, not after.** `backslop gates` runs the commands from `gates` in `backslop.json` in order, takes the exit code from the command itself rather than from a pipe, and prints “gates N, green M” together with a tree snapshot; `--require-clean` refuses on a dirty tree before the first command, and `--json` returns the same content for a report. What path filtering skips is counted separately and is not green: a command whose paths the change did not touch is not run, so “gates N, green M” on its own is no longer the criterion. A red gate in a reported result costs an entire round-trip.{{probeRule}} A mutation probe is deliberate breakage: break the code and make sure the test fails. **The commit comes first:** `git checkout <file>` removes all uncommitted work, not just one mutation. If the probe remains green, first ask whether the code path is live: a check that cannot be broken is dead code, not a coverage gap. If the path is live and the probe is green, the check input does not reach the property it claims to verify; fix the input, not the gate. Run gates on an unchanged tree: editing files during a run produces a red result with a misleading diagnosis; `backslop gates --require-clean` checks that mechanically, and the snapshot at the end names the commit and whether the tree is clean.
...
- gate results as numbers, not merely “green”: how many checks, how many files, how many were skipped by path; what path filtering skips does not add to green. Take the number from the summary line of `backslop gates` or from its `--json`, not from memory;
...
- findings outside the assigned scope — a neighbouring bug, a gap in rules, an unanswered question — with a cost label: fix `critical` now within your boundaries, in another track's files message the orchestrator at once; fix `major` within your task's “Scope” now and name it here; file `major` outside it yourself, in the worker branch: `backslop new <slug> --parent N[.M]` puts a file in `triage/`; file `minor` and hypotheses with `backslop new <slug> --parent N[.M] --minor --evidence "…"` into `minor/`. Status directories stay untouched. A verifiable fact in a finding includes evidence — a command or a file and line; grep takes seconds when already in the repository. Not verified means a hypothesis, and its place is `minor/` with `--cost <level> --hypothesis`; it carries `--evidence` just like a verified finding, only worded as an assumption — without it the command refuses before the file is created.
...
2. Complete `result.md`: outcome (completed, rejected, or merged), exactly what was done, verification with numerical gates, mutation probe and live run, and documentation updated. While a `[TODO` placeholder remains outside code, `backslop lint` fails — that is the reminder; the placeholder form quoted in a code span does not count.
