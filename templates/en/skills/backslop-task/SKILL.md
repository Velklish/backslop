...
4. **Run gates before reporting, not after.** `backslop gates` runs the commands from `gates` in `backslop.json` in order, takes the exit code from the command itself rather than from a pipe, and prints “gates N, green M” together with a tree snapshot; `--require-clean` refuses on a dirty tree before the first command, and `--json` returns the same content for a report. A red gate in a reported result costs an entire round-trip. Verify a test change with a mutation probe: break the code and make sure the test fails. **Commit first, then probe:** `git checkout <file>` removes all uncommitted work, not just one mutation. If the probe remains green, first ask whether the code path is live: a check that cannot be broken is dead code, not a coverage gap. If the path is live and the probe is green, the check input does not reach the property it claims to verify; fix the input, not the gate. Run gates on an unchanged tree: editing files during a run produces a red result with a misleading diagnosis; `backslop gates --require-clean` checks that mechanically, and the snapshot at the end names the commit and whether the tree is clean.
