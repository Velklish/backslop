...
3. **`{{docs}}/adr/`** — backfill selected decisions: `backslop adr <slug> --title "…"`, status `Accepted`, original date from history, and a “recorded retrospectively” note in Context; put the owner in `adr-001-process.md` Deciders. Each ADR gets a row in `{{docs}}/README.md`. Rationale only with evidence; when history is sparse, say so in Context.
