...
2. You resolve conflicts. They are usually in files shared by tracks, and **a merge strategy cannot resolve them**: `-X theirs` and `-X ours` take one side’s file whole and silently drop the neighbouring track’s hunks — CHANGELOG entries, reference sections, list items in code — or duplicate the file’s tail. Build a shared file from both revisions (`git show HEAD:<file>` and `git show <worker-head>:<file>`) by the units of its structure, not by resolving conflict markers:
   - **CHANGELOG** — run `{{cli}} merge-changelog --ours <your head> --theirs <worker head> [--base <branch point>] --out CHANGELOG.md` from the project root: it merges unreleased entries by their `- **…**` heading, keeps the section's structure — a `### …` heading and the bold subgroup under it — and takes released sections from your side: their headings repeat in each release and there is nothing to merge. The command does not pick between diverging bodies under one heading: both revisions stay under a `<!-- backslop:conflict … -->` mark, and gate 7 keeps `lint` red until you decide. **The command may hand over no file at all**: it refuses on an unresolved mark in an input revision and on a broken invariant of its own check, and `--out` is left untouched. A non-zero exit code means an unresolved conflict or a refusal — read the code from the command, not the presence of a file. Without `--base` an entry removed by one side is indistinguishable from a neighbour’s new one — it is kept, and the command names it in the report together with one-sided bullets that carry no bold heading. The unreleased section is the first heading that does not start with a version number (a leading `v` or `[` is allowed), or, when there is none, the top section whose version has no tag: that is how a bump leaves it before the release. Your side decides where entries land, not the time of the bump: while no such heading stands above the bumped section, track entries are merged into the bumped version, and after the tag the command refuses until such a heading exists. To send entries to the next version, add `## Unreleased` above the bumped section on your side and merge with `--base`: without a base, entries the bumped section already carries arrive from the worker a second time. The report line “unreleased section in <side> — “…”: the version has no tag” means the merge went into a version section: check that the version is not released; if it is, the tag has not reached this clone — run `git fetch --tags` and repeat. The line “theirs has no unreleased section — its entries were not read” means the worker's entries are not in the result: find them in its revision;
...
4. Acceptance is in that commit: `backslop archive N`, `result.md`, then `backslop fold N > "$(git rev-parse --git-dir)/BACKSLOP_DRAFT"` — the directory folds into a journal line, and the task body goes into a commit message draft outside the working tree; fold a batch after its entries, as described above. Do not commit between `archive N` and `fold N` — except for a track leaving as one commit, see below. The acceptance commit carries the draft as its message: `git add -A`, `git reset --soft <base>` if the merge is already committed, and `git commit -F "$(git rev-parse --git-dir)/BACKSLOP_DRAFT"`; you may replace the subject with the form from step 1, but keep the body — it is no longer in the tree. A `fixup` or squash after this commit discards the draft. `backslop lint` is green on this commit, before the push. **A track of several tasks leaving as one commit (step 1) is not folded in that commit:** a commit has one draft, the second `fold N > …` overwrites the first, and the first task's directory leaves the tree together with its `result.md` without ever reaching history. That commit carries the archive directories — `backslop archive N` and `result.md` for each task, without `fold`. Folding goes in the next commit: `backslop fold N` for each task, or a bulk `backslop fold`. By then the directories are in history, the journal line gets their revision, and the message draft is not required there (ADR-033). Run gates before committing, on an unchanged tree.
