...
After distributing briefs, **move every assigned task to active work** in one call: `backslop mv N M K active`. You hold directories; workers do not touch them at all. When the run ends and you have not accepted work, return the whole track to the queue in one call: `backslop mv N M K queue --restore`. Each task goes back where it left from as far as the queue allows, and the order of numbers in the command does not change the outcome. Need another priority — use `--top` or `--after M`, but they take one number. Without `--restore` the saved position is lost, and the command says so.
