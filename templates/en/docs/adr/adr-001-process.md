...
- gates are `{{cli}} lint` plus `gates` from `backslop.json`;
- the tool version is pinned in `backslop.json` (`cli` with a tag and the `version` stamp); update with `{{cli}} upgrade`, while `migrate` changes file formats between versions.

## Consequences

- Changing status is a `git mv` of one file: two branches conflict only on the same task.
- There is no task list in git — `{{cli}} status` provides the summary; opening the backlog README to see the queue is pointless.
