...
|---|---|---|
| `triage/` | Unreviewed ideas and findings. The file is the entry | `{{cli}} new <slug> --title "…"`; a `major` finding outside the current task's scope is `{{cli}} new <slug> --parent N[.M]` |
...
| `minor/` | Minor findings and hypotheses: they wait for a batch, not for review; the “Cost” field is required, “Scope” is filled in when batches are cut | `{{cli}} new <slug> --parent N[.M] --minor [--cost <level>] [--hypothesis]`; `{{cli}} mv N minor` |
...
- When an idea appears, add an entry in `triage/`: `{{cli}} new <slug>` writes a card from the template, and nothing has to be filled in before review. A line or two under “Context” is enough. The `[TODO]` placeholders stay as they are — the placeholder gate does not look into `triage/`. Review is a separate pass.
...
- **“Scope”** links to a [reference/](../reference/README.md) section: in `queue/`, `active/`, and `deferred/` an empty field or a field whose entire value is a `[TODO…]` placeholder left by `{{cli}} new` is a `lint` error; in `triage/` the field is not checked — it is filled in during review; in `minor/` an empty field is a warning, and the approver fills it in when cutting batches. A standalone `[TODO…]` placeholder line in any markdown file under `docs/backlog/**` also fails `lint` — except in `triage/`, where placeholders are not checked at all: the card sits unfilled until review. `[TODO]` inside explanatory text, as part of a larger value, is not a placeholder. The `Evidence:` line of a finding from `{{cli}} new <slug> --parent N[.M]` falls under the same exception: unfilled, it no longer fails the gate — the approver asks for the evidence during review. In `minor/` that line still fails the gate.
- **Priority = the “Order” field** in `queue/`. Reorder with `{{cli}} mv N queue --top` or `--after M`, including a task already in the queue: the file stays, only the number changes. To return a task where it left the queue from, use `--restore`: the departure stores that number in the “Previous order” field. The flag takes several numbers per call; a return without it discards the saved position and says so. Two files with one “Order” is a `lint` error.
- **Closure** — completed, rejected, or merged — uses `{{cli}} archive N`: the file moves to the archive as `task.md`, alongside a dated `result.md`, and the output names the documentation files touched by the task (`--range <base>..HEAD` widens the selection with the range's commits). The approver completes the outcome and result; while `result.md` contains `[TODO]`, `lint` fails. The directory is then folded into a journal line — `{{cli}} fold N`: nothing of it stays in the tree, incoming links move onto the line anchor, and the definition and the result go in full into the commit message draft the command prints on stdout. Committing with that draft is mandatory: the body is no longer in the tree, and `{{cli}} show N` retrieves it. Folding is not mandatory — a directory remains a legal form of the record. A `minor/` entry is closed by a batch: `{{cli}} archive N.k --into M` moves its file into `archive/<M>-<slug>/minor/` without a `result.md` of its own, and its outcome is one line in the batch's `result.md`; the batch is closed first.
...
**Minor batches are cut at the same point, before the run.** The approver walks `minor/` — `{{cli}} status` prints it by scope — fills in empty “Scope” fields and assembles one batch card per scope: the scope the run touches anyway, or one with ten or more entries. A batch is an ordinary card, `{{cli}} new <slug> --queue --title "…"`, listing the numbers under “Work to do”. The owner may order a batch at any time. Closing batch M: first `{{cli}} archive M`, then `{{cli}} archive N.k --into M` for each entry, and their outcomes as lines in the batch's `result.md`; `{{cli}} fold M` comes last and carries the entries away with the batch — each gets its own journal line with the outcome “batch M”.
