// Слияние двух редакций CHANGELOG по заголовку, с подгруппами и базой. Команда — настоящим
...
// Секция с `### …`, буллетом без жирного заголовка и записями по обе стороны от него — форма,
// на которой слияние дублировало заголовок, выдумывало расхождение тел и переразмечало файл.
...
test('merge-changelog: самопроверка отказывает и файл не отдаётся', () => {
  // Одна жирная подгруппа у сторон под разными заголовками встала бы в результате дважды —
  // слияние отказывается, а не отдаёт файл с выросшей структурой.
  const ours = '# Changelog\n\n## Не выпущено\n\n### Добавлено\n\n**Для агента:**\n\n- **Одна** — ours\n';
  const theirs = '# Changelog\n\n## Не выпущено\n\n### Исправлено\n\n**Для агента:**\n\n- **Две** — theirs\n';
