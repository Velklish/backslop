...
test('список долга умеет только уменьшаться', () => {
  ...
  assert.ok(ids <= 177, `список должен ${ids} блоков, потолок — 177`);
});
...
test('обход говорит, сколько прочитал и сколько строк это стоит', () => {
  ...
  assert.equal(walk.seen, 177, `обход увидел ${walk.seen} длинных блоков, дерево известно как несущее 177`);
  ...
  assert.equal(walk.lines, 692, `блоки заняли ${walk.lines} строк, дерево известно как несущее 692`);
});
