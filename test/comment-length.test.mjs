...
test('список долга умеет только уменьшаться', () => {
  ...
  assert.ok(ids <= 180, `список должен ${ids} блоков, потолок — 180`);
});
...
test('обход говорит, сколько прочитал и сколько строк это стоит', () => {
  ...
  assert.equal(walk.seen, 180, `обход увидел ${walk.seen} длинных блоков, дерево известно как несущее 180`);
  ...
  assert.equal(walk.lines, 705, `блоки заняли ${walk.lines} строк, дерево известно как несущее 705`);
});
