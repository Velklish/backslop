...
test('список долга умеет только уменьшаться', () => {
  ...
  assert.ok(ids <= 174, `список должен ${ids} блоков, потолок — 174`);
});
...
test('обход говорит, сколько прочитал и сколько строк это стоит', () => {
  ...
  assert.equal(walk.seen, 174, `обход увидел ${walk.seen} длинных блоков, дерево известно как несущее 174`);
  ...
  assert.equal(walk.lines, 677, `блоки заняли ${walk.lines} строк, дерево известно как несущее 677`);
});
