...
test('fold N: тело с ревизией — заготовка не обязательна, show N достаёт тело; без ревизии — обязательна', () => {
  ...
  try {
    ...
    // Ход приёмки: archive N и result.md без коммита — ревизии нет,
    // заготовка единственное хранилище.
...
// Коммит с заданной датой: день свёртки и дата ревизии иначе совпали бы,
// и проверка была бы холостой.
...
// Правка, которую `git status` не видит: чистый по статусу каталог,
// а на диске — не то, что в ревизии.
...
test('show N: git, оборванный на чтении сообщения, — отказ называет сигнал, а не «код null»', { skip: process.platform === 'win32' }, () => {
  const root = makeProject();
