...
```
<проект>/
  backslop.json                    конфиг: prefix, docs, cli, gates
...
Правила идемпотентности ([lib/init.js](../../lib/init.js)): конфиг создаётся один раз, и флаги, расходящиеся с ним, — отказ; файлы `docs/` создаются только отсутствующие; скиллы переписываются на версию из шаблонов при каждом запуске; блок в `AGENTS.md` заменяется между маркерами, а без маркеров дописывается в конец. Запуск в подкаталоге уже инициализированного проекта отказывает и называет корень.
...
| `cli` | `npx github:Velklish/backslop` | как звать backslop из этого проекта; подставляется в скиллы и блок AGENTS.md |
