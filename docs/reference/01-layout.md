...
Механическую половину строки «Доки тем же ходом» `archive` печатает: файлы под `<docs>/` и `CHANGELOG.md`, которых коснулись коммиты диапазона `--range` и коммиты с номером задачи в заголовке (`<prefix>-N: …`). Список только печатается — что из него попадёт в `result.md`, решает approver; «Исход» и «Проверки» не подставляются вовсе, это его суждение по существу задачи.
