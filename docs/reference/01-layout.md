...
Правила идемпотентности ([lib/init.js](../../lib/init.js)): конфиг создаётся один раз, и флаги `dir`/`prefix`/`cli`, расходящиеся с ним, — отказ; `--lang` и `--tools` на повторном `init` меняют конфиг. Файлы `docs/` создаются только отсутствующие; выбранные adapter outputs переписываются, снятые чистятся по предикату владения — маркер в файле или legacy-путь, а не совпадение с текущим шаблоном; чужой файл на пути adapter output остаётся и называется предупреждением — это про снятие; выбранный adapter пишет свои файлы поверх того, что лежало на их путях; блок в `AGENTS.md` заменяется между маркерами, а без маркеров дописывается в конец; штамп `version` переставляется на версию инструмента, пин в `cli` при этом не трогается — расхождение называется предупреждением. Запуск в подкаталоге уже инициализированного проекта отказывает и называет корень. Пути корней трёх harness — `.claude/skills`, `.cursor/rules`, `.agents/skills` — проверяются на symlink до первой записи и независимо от `tools`: бракуется symlink на любой компоненте пути под корнем проекта, в том числе на самом `.claude`. Отказ случается, пока на диске нет ни конфига, ни скелета; лечение — обычный каталог на месте компоненты, `--tools none` ситуацию не меняет. Self-host репозитория держит `tools: []`: обычный `init` не создаёт harness-файлов и не делает tracked tree грязным.
