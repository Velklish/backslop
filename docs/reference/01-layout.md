...
Поле шапки — строка `- **Имя:** значение` между заголовком и первым разделом `## `; порядок полей произвольный, «Порядок» команды ставят первым ([lib/tasks.js](../../lib/tasks.js), `setField`). Каждое известное поле, включая его RU/EN-алиасы, должно встречаться в файле один раз: чтение и запись берут первое вхождение по порядку строк, `setField` схлопывает остальные, а `removeField` удаляет все. Заголовок первой строки обязан называть тот же номер, что имя файла.
