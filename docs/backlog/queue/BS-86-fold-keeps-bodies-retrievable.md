...
- Bug 3: make the stored body survive `commit.cleanup=strip`: prefix every embedded body line with a fixed marker that `show` strips back (or indent by one space), and document `git commit --cleanup=verbatim -F` next to the fold recipe in docs/reference/02-cli.md and in the fold note; `show` warns when the retrieved body has no `# <id> ·` first line.
