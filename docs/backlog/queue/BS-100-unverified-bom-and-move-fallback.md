...
- [ ] BOM kept on card rewrite (lib/tasks.js)
    ...
    2. Refutation check: docs/reference/02-cli.md:37 says task files "are read without BOM, line endings are preserved" and lib/tasks.js:346-347 says the same for EOL only. If the owner reads this as "BOM is normalised away on write", record the sanction, state it explicitly in 02-cli.md, and stop.
    3. Fix only if confirmed: remember the BOM in `readText` (return it with the text or keep a flag) and re-emit it in `writeText`; test that `mv` and renumbering keep `ef bb bf`.
