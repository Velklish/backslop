...
- Bug 4: Owner decision: block markers are recognised only as whole lines. Locate the managed block by line-anchored markers (each marker alone on its line); a marker text inside prose outside the block is harmless, and the block is updated. When either marker occurs more than once as such a line, refuse with the existing unpaired-marker style CliError instead of rewriting; an unpaired whole-line marker is still refused.
...
- New test (bug 4): AGENTS.md with a prose line quoting both markers before the block → `init` rc=0; the prose line is kept unchanged and exactly one block remains, equal to the rendered block: `grep -cx '<!-- backslop:start -->' AGENTS.md` → 1, `grep -cx '<!-- backslop:end -->' AGENTS.md` → 1.
