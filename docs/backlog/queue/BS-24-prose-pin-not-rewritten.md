...
- **Создана:** 2026-09-06
- **Зависимости:** нет
- **Слито сюда:** BS-27 (та же дыра, улика из promptobus)
...
- Фикстура из слитой BS-27: расхождение внутри одного файла — версия названа прозой на одной строке, пин командой на соседней (promptobus, `docs/guides/contributing.md:3` — «run entirely through backslop `v0.3.0`» против `#v0.4.0` на строках 6-7). Замена по литералу `cli` этот случай не покрывает: прозаический `v0.3.0` литералу `npx github:Velklish/backslop#v0.3.0` не равен. Решение — по выбору исполнителя: либо второй шаблон поиска (голый `vX.Y.Z` рядом со словом backslop), либо явный отказ покрывать этот случай, названный в reference.
...
- На подставном проекте с docs/archive/README.md, содержащим старый пин: `upgrade` (полный, не `--pin-only` — см. «Что сделать») переписывает файл на новый пин; docs/adr/... и docs/archive/<prefix>-N-*/task.md остаются прежними.
