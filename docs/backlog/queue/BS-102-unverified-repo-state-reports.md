...
- [ ] Project scope of `--require-clean` (lib/gates.js)
    ...
    2. Owner decision: in a monorepo, `gates --require-clean` and the `tree.dirty` snapshot consider only the project area; this is a fix, not a sanctioned behaviour.
    3. Fix only if step 1 reproduces: filter the snapshot through the project prefix (`git status --porcelain -- .` run from the project root), so `--require-clean` and `tree.dirty` refer to the project; report tree paths relative to the project, like `changedPaths`, and for a rename `R a -> b` rewrite both sides; state the project scope in the gates row of docs/reference/02-cli.md; test in a monorepo fixture.
...
- For each item the result names the outcome of steps 1–2 with the command output and rc (item 1: step 1 only; its step 2 is the owner decision).
- Each confirmed item has a red-then-green test; a sanctioned item (item 2 only) has a doc sentence instead of a code change.
- Item 1 after the fix, in the step-1 monorepo fixture: `gates --require-clean --base HEAD~1` does not refuse on `?? other/b.txt` (rc=0); an untracked file inside `pkg/` makes it refuse with that file's path relative to the project; a staged rename inside `pkg/` is listed with both sides relative to the project; `gates --json` shows the same lines in `tree.dirty`.
