...
- Pin rule, stated generally: every command and flag the brief tells the worker to run must exist at the version the project pins, because the brief is often rendered by a newer CLI than the pin. Record how the code enforces it after the BS-90 (`brief-follows-cli-pin`) card.
