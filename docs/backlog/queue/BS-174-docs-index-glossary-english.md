...
- **Created:** 2026-09-25
- **Dependencies:** BS-157, BS-140, BS-159, BS-160, BS-143, BS-144, BS-145, BS-146, BS-147, BS-148, BS-149, BS-150, BS-151, BS-152, BS-153, BS-154, BS-155, BS-156
...
- ADR table: exactly one row per file in `docs/adr/` as left by the consolidation — English topic, Status equal to the Status line in the file, no supersession narration (evidence item 7). Principles: translate 1-4 using the wording of `templates/en/docs/README.md:14-17`; principle 2 keeps its rule for future decisions (the consolidated set is the baseline, no current ADR is superseded); principle 5 becomes one sentence linking `AGENTS.md` (evidence item 11); the closing line reads: every ADR must be linked from this file — lint gate 8 checks the link — and the link is kept as a row of the table above (evidence item 6).
...
- `docs/reference/README.md` in English if it is still Russian, listing pages 01-03 (the BS-176 (`orchestrator-contract-reference`) and BS-177 (`lib-module-map-reference`) cards add 04 and 05). Archive row of `templates/docs/README.md:11` and `templates/en/docs/README.md:11`: both forms (same meaning in each language) — the BS-166 (`docs-skeleton-adr001-index`) card already changed this row in both templates; check that it describes both forms.
