...
- `--dry-run` (из слитой BS-28) печатает перечень гейтов, ничего не исполняя — так проверяется состав `gates` перед прогоном
...
- Правки на стороне потребителей (из слитой BS-28: `docs/guides/contributing.md:31,79` в promptobus дублирует список гейтов прозой и обещает audit-проверки «when they land», хотя `npm run audit` уже в `gates`) — чужой репозиторий, отдельный коммит в его собственном бэклоге, не в этом
