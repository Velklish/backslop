...
The owner decided that `docs/ROADMAP.md` leaves this repository and the tool layout. Templates in both languages go, `init` stops creating the file, and `migrate` deletes a consumer copy only while it still equals the rendered template. This changes the layout contract. Owner decision: no separate ADR; the ROADMAP decision becomes a section of the consolidated version-pin/upgrade/migrate ADR written by BS-152 (`adr-pin-upgrade-migrate`).
...
- lib/migrate.js: add a MIGRATIONS entry with `since: '0.12.0'` and a `{ en, ru }` title such as "docs/ROADMAP.md leaves the layout". It must: (a) keep the frozen v0.9.0–v0.11.0 ROADMAP text for ru and en, plus the old rendered docs/README.md line 3 and line 9, as constants in lib/ (the template files are gone); (b) render them with `{ project: projectName(root), cli: cfg.cli }` for `cfg.lang`; (c) compare after normalizing `\r\n` to `\n` and every pin occurrence (`pinRe(parseCli(cfg.cli))`, any version) to the current cli pin on both sides, as BS-84 (`upgrade-completes-pinned-consumers`) does for the rules pair in `planRules`; (d) delete `docs/ROADMAP.md` only if it equals the render and no markdown file links it apart from the two old docs/README.md lines; (e) Owner decision: the migration edits line 3 and line 9 of the project's docs/README.md only where the line equals its old rendered form: it replaces such a line 3 with the new rendered line 3 and drops such a line-9 row; (f) refuse on an uncommitted edit of either file and skip paths through a symlink, the way `planRules` does (migrate.js:47-70); (g) in every other case keep the files and warn, naming ROADMAP.md and every file that links it, e.g. "kept: differs from the template or is still linked — delete it yourself or keep it as project content". With `--dry-run` it only prints what it would do.
- Owner decision: v0.12.0 is released when this card is accepted.
- Release mechanics (approver and orchestrator, not the worker): the worker does not bump the version. Until the bump, test/version.test.mjs (every migration's `since` is at most the tool version) is red on the worker's branch; the worker names this expected red in the result with its assert text. At acceptance the approver runs `npm run release -- 0.12.0 --bump` on the merged tree before the gates, so the card's commit carries the bump. After that commit the orchestrator runs `npm run release -- 0.12.0 --no-publish` and opens a new `## Unreleased` above `## v0.12.0 — <date>`.
- ADR section: add a section on ROADMAP.md to the consolidated version-pin/upgrade/migrate ADR written by BS-152 (`adr-pin-upgrade-migrate`); no new ADR file and no new docs/README.md row. No task numbers, no old ADR numbers, no dated owner-decision note, and do not invent a rationale the owner did not give. Context: the tool layout no longer includes a roadmap file, and the mechanics above (the project-owned docs/README.md and the redrawn backlog README). Options: keep the file; drop only the templates (consumer lint turns red, as in the repro); drop the templates and add the delete-if-unchanged migration. Decision: the third. Consequences: an edited copy stays as project content, and the migration runs once per stamp.
...
- Rewriting the consolidated ADRs beyond the ROADMAP section and the skeleton list of the version-pin/upgrade/migrate ADR.
...
- `ls docs/adr | grep -c -i roadmap` prints 0 (no separate ADR); the consolidated version-pin/upgrade/migrate ADR carries the ROADMAP section and no ROADMAP.md in its skeleton list.
- On the worker's branch: `npm test` exits 1 with exactly one failing test, the `since` check in test/version.test.mjs, whose assert names the new migration, `since 0.12.0` and the tool version 0.11.0; every other test passes (450 at the base commit, plus the tests this card adds). After the approver's `npm run release -- 0.12.0 --bump`: `npm test` exits 0 with no failing test. `node bin/backslop.js lint` exits 0 before and after the bump (templateParity and the key check stay clean).
