...
- **Создана:** 2026-09-06
- **Зависимости:** нет
- **Слито сюда:** BS-41 (тот же дрейф пина в README, разбор примеров установки)
...
- Заодно поправить `#v0.2.0` в `README.md:59,63` и `README.ru.md:59,63` — новый гейт иначе сразу красный на существующем дереве. Из слитой BS-41: в примере установки живёт не только устаревшее число, но и неверное описание поведения — `defaultCli()` (`lib/config.js:20`) подставляет текущий `TOOL_VERSION`, а `docs/reference/01-layout.md:35` ту же строку уже пишет обобщённо, `#v<версия>`. Поэтому в примерах установки README ставится обобщённая форма (дрейфовать нечему), а гейт сторожит те вхождения, где номер обязан быть настоящим.
...
- Из слитой BS-41: `grep -n "v0\.2\.0" README.md README.ru.md` — пусто; обобщённая форма `#v<версия>` стоит в обоих README в примерах установки.
