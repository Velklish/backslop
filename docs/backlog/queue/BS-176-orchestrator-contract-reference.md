...
- Stability section: the explicit stable list, extended with the items below. Owner decision: the list is confirmed — `brief` output and its slots, `gates --json`, `tracks --json`, `seed --scan --json`, `agents.stepOverrides`, the exit codes of `merge-changelog`, and the outcome discriminator (rc=1 with JSON on stdout is a result, rc=1 with empty stdout is a refusal) are stable; additive JSON keys are allowed, removing or renaming a key is breaking; human text is not contract. Add a "Not contract" list: human text, glyphs, colours, info-line order, JSON key order, stderr reports, placeholder wording after its TODO marker, lint message text.
- One ADR rule in the stability section (item 12). Owner decision: the rule stays as `AGENTS.md` states it — a behaviour change of a command updates `docs/reference/`, README and CHANGELOG in the same pass; a contract change (file formats, commands, the composition of `status --json`) needs a new ADR.
- Move out of `docs/reference/02-cli.md`: the `status --json` and "What is stable" sections, the exit-code/glyph sentences of the intro (line 3), the JSON and exit sentences of the `gates` row (line 23), the JSON part of the `tracks` (24) and `seed --scan` (20) rows, the contract part of the `brief` row (19); leave one-line pointers. Add a row for page 05 to `docs/reference/README.md`.
- Links to the new page: one line in `README.md` § "For orchestrators" (replacing the pointer to 02-cli § What is stable), the contract-change line of `AGENTS.md` points at the stability section, a row for page 05 in `docs/reference/README.md`, and the `reference/` row of `docs/README.md` names it.
...
- `grep -nE 'ADR-[0-9]|BS-[0-9]' docs/reference/05-orchestrator-contract.md; echo rc=$?` → rc=1 (no matches).
- The stability section names every item of the owner-confirmed list and states the ADR rule in the words of `AGENTS.md`.
